...
impl From<&CoreLibrary> for HostLibrary {
    fn from(libcore: &CoreLibrary) -> Self {
        Self {
            mast_forest: libcore.mast_forest().clone(),
            handlers: libcore.handlers(),
        }
    }
}
...
/// expected by `core::crypto::dsa::ecdsa_k256_keccak::verify` procedure.
///
...
pub fn eddsa_sign(sk: &eddsa_25519_sha512::SecretKey, msg: Word) -> Vec<Felt> {
    let pk = sk.public_key();
    let sig = sk.sign(msg);
...
pub fn encode_eddsa_signature(
    pk: &eddsa_25519_sha512::PublicKey,
    sig: &eddsa_25519_sha512::Signature,
) -> Vec<Felt> {
    let mut out = Vec::new();
    let pk_bytes = pk.to_bytes();
    out.extend(bytes_to_packed_u32_felts(&pk_bytes));
