use crate::{ExecutionError, Process, chiplets::eval_circuit};
...
impl Process {
    ...
    pub fn arithmetic_circuit_eval(&mut self) -> Result<(), ExecutionError> {
        ...
        let circuit_evaluation =
            eval_circuit(ctx, ptr, clk, num_read_rows, num_eval_rows, &mut self.chiplets.memory)?;
