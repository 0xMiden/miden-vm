...
/// the key `KEY` located at the top of the stack. Any existing values for that key are overwritten.
///
/// Inputs:
...
fn insert_mem_values_into_adv_map(process: &mut ProcessState) -> Result<(), ExecutionError> {
    ...
    process.advice_provider_mut().map_mut().insert(key, values);
    Ok(())
}

/// Reads two word from the operand stack and inserts them into the advice map under the key
/// defined by the hash of these words. Any existing value for that key is overridden.  
///
/// Inputs:
///   Operand stack: [B, A, ...]
///   Advice map: {...}
...
/// under the key defined by applying an RPO permutation to all three words. Any existing value for
/// that key is overridden.
///
/// Inputs:
///   Operand stack: [B, A, C, ...]
///   Advice map: {...}
