...
macro_rules! require_u32_operands {
    ($stack:expr, [$($idx:expr),*]) => {
        require_u32_operands!($stack, [$($idx),*], ZERO)
    };
    ($stack:expr, [$($idx:expr),*], $errno:expr) => {{
        paste!{
...
impl Process {
    ...
    pub(super) fn op_u32split(&mut self) -> Result<(), OperationError> {
    ...
    pub(super) fn op_u32assert2(&mut self, err_code: Felt) -> Result<(), OperationError> {
        let (b, a) = require_u32_operands!(self.stack, [0, 1], err_code);
    ...
    pub(super) fn op_u32add3(&mut self) -> Result<(), OperationError> {
        let (c, b, a) = require_u32_operands!(self.stack, [0, 1, 2]);
    ...
    pub(super) fn op_u32madd(&mut self) -> Result<(), OperationError> {
        let (b, a, c) = require_u32_operands!(self.stack, [0, 1, 2]);
        let result = Felt::new(a * b + c);
        let (hi, lo) = split_element(result);
    ...
    pub(super) fn op_u32and(&mut self) -> Result<(), OperationError> {
        let (b, a) = require_u32_operands!(self.stack, [0, 1]);
        let result = self.chiplets.bitwise.u32and(Felt::new(a), Felt::new(b))?;
    ...
    pub(super) fn op_u32xor(&mut self) -> Result<(), OperationError> {
        let (b, a) = require_u32_operands!(self.stack, [0, 1]);
        let result = self.chiplets.bitwise.u32xor(Felt::new(a), Felt::new(b))?;
