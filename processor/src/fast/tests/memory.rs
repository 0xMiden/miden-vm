...
fn test_memory_word_access_alignment() {
    ...
    // mloadw
    {
        ...
        FastProcessor::new(&[Felt::new(40)]).execute_sync(&program, &mut host).unwrap();

        // but loadw at address 43 is not allowed
        let err = FastProcessor::new(&[Felt::new(43)])
...
fn test_mloadw_success() {
    let mut host = DefaultHost::default();
    let addr = Felt::new(40);
    let word_at_addr = [Felt::new(1), Felt::new(2), Felt::new(3), Felt::new(4)];
    let ctx = 0_u32.into();
    ...
    {
        let mut processor = FastProcessor::new(&[Felt::new(100)]);
        processor
            .memory
            .write_word(ctx, addr, dummy_clk, word_at_addr.into(), &())
            .unwrap();
...
fn test_mstorew_success() {
    let mut host = DefaultHost::default();
    let addr = Felt::new(40);
    let word_to_store = [Felt::new(1), Felt::new(2), Felt::new(3), Felt::new(4)];
    let ctx = 0_u32.into();
    let clk = 0_u32.into();
...
fn test_mstream() {
    let mut host = DefaultHost::default();
    let addr = 40_u32;
    let word_at_addr_40 = Word::from([ONE, Felt::new(2), Felt::new(3), Felt::new(4)]);
    let word_at_addr_44 = Word::from([Felt::new(5), Felt::new(6), Felt::new(7), Felt::new(8)]);
    let ctx = 0_u32.into();
    let clk = 1_u32.into();
