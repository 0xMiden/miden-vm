...
fn test_mloadw_success() {
    ...
    // load the contents of address 40
    {
        ...
        processor.memory.write_word(ctx, addr, word_at_addr.into()).unwrap();
...
fn test_mstorew_success() {
    ...
    assert_eq!(processor.memory.read_word(ctx, addr).unwrap(), word_to_store.into());
}
...
fn test_mstore_success(#[case] addr: u32, #[case] value_to_store: u32) {
    ...
    let word = processor.memory.read_word(ctx, word_addr.into()).unwrap();
...
fn test_mload_success(#[case] addr_to_access: u32) {
    ...
    processor
        .memory
        .write_word(ctx, addr_with_word.into(), word_at_addr.into())
        .unwrap();
