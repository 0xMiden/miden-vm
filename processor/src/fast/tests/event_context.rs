...
fn fast_processor_adapter_supplies_context_capabilities() {
    ...
    assert_eq!(context.memory_value(4).unwrap(), felt(200));
    assert_eq!(context.memory_word_root(4).unwrap(), word(100));

    let mut memory_output = [ZERO; 2];
    processor.read_memory_root(MemoryAddress::new(5), &mut memory_output).unwrap();
    ...
    processor.memory.write_element(active_context, felt(17), felt(400)).unwrap();
    processor.memory.write_element(root_context, felt(22), felt(500)).unwrap();
    let stored_words = processor.memory.num_accessed_words();
    let context = EventContext::new(&processor, Invocation::event(EventId::from_u64(1), 0, false));
    assert_eq!(context.memory_word(0).unwrap(), Word::default());
    assert_eq!(context.memory_word_root(0).unwrap(), Word::default());
    assert_eq!(context.memory_word(16).unwrap(), Word::new([ZERO, felt(400), ZERO, ZERO]));
    assert_eq!(context.memory_word_root(16).unwrap(), Word::default());
    assert_eq!(context.memory_word(20).unwrap(), Word::default());
    assert_eq!(context.memory_word_root(20).unwrap(), Word::new([ZERO, ZERO, felt(500), ZERO]));
    for (start, count) in [
        (0, 4),
        (0, 20),
        (3, 2),
        ...
        let expected: Vec<_> = (start..start + count)
            .map(|address| context.memory_value(address).unwrap())
            .collect();
        let expected_root: Vec<_> = (start..start + count)
            .map(|address| context.memory_value_root(address).unwrap())
            .collect();
        ...
        assert_eq!(context.memory_range(start, start + count).unwrap(), expected);
        assert_eq!(context.memory_range_root(start, start + count).unwrap(), expected_root);
        if count == WORD_SIZE as u64 && start.is_multiple_of(WORD_SIZE as u64) {
            assert_eq!(context.memory_word(start).unwrap().as_elements(), expected.as_slice());
            assert_eq!(
                context.memory_word_root(start).unwrap().as_elements(),
                expected_root.as_slice()
            );
        }
    }
    // Reading zero-valued cells must not allocate backing words.
    assert_eq!(processor.memory.num_accessed_words(), stored_words);

    // Range overflow must leave both current- and root-context provider buffers intact.
    let mut output = [felt(99); 6];
    assert_eq!(
        processor.read_memory(MemoryAddress::new(u32::MAX), &mut output),
        Err(EventContextError::RangeOverflow { start: u64::from(u32::MAX), count: 6 })
    );
    assert_eq!(output, [felt(99); 6]);
    assert_eq!(
        processor.read_memory_root(MemoryAddress::new(u32::MAX), &mut output),
        Err(EventContextError::RangeOverflow { start: u64::from(u32::MAX), count: 6 })
    );
    assert_eq!(output, [felt(99); 6]);
}
...
fn legacy_bridge_discards_child_writes_when_the_host_catches_failure() {
    ...
    host.register_handler(
        ...
        Arc::new(move |state: &ProcessorState<'_>| {
            ...
            Ok(vec![AdviceMutation::extend_advice_stack_with([felt(7)])])
        }),
    )
    .unwrap();
    let program = Assembler::default()
