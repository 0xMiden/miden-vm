...
fn create_simple_library() -> HostLibrary {
    ...
    let swap_block = mast_forest.add_block(vec![Operation::Swap, Operation::Swap], None).unwrap();
...
fn join_program() -> Program {
    ...
    let basic_block_mul = program.add_block(vec![Operation::Mul], None).unwrap();
    let basic_block_add = program.add_block(vec![Operation::Add], None).unwrap();
    let basic_block_swap = program.add_block(vec![Operation::Swap], None).unwrap();
...
fn split_program() -> Program {
    ...
    let root_join_node = {
        let basic_block_swap_swap =
            program.add_block(vec![Operation::Swap, Operation::Swap], None).unwrap();
    ...
    let root_join_node = {
        ...
        let target_loop_node = {
            let basic_block_pad_drop =
                program.add_block(vec![Operation::Pad, Operation::Drop], None).unwrap();
...
fn basic_block_program_small() -> Program {
    ...
    let root_join_node = {
        let target_basic_block = program
            .add_block(vec![Operation::Swap, Operation::Push(42_u32.into())], None)
            .unwrap();
        let basic_block_drop = program.add_block(vec![Operation::Drop], None).unwrap();
...
fn basic_block_program_multiple_batches() -> Program {
    ...
    let root_join_node = {
        let target_basic_block = program.add_block(vec![Operation::Swap; NUM_SWAPS], None).unwrap();
        let basic_block_drop = program.add_block(vec![Operation::Drop], None).unwrap();
...
fn dyn_program() -> Program {
    ...
    let target = program.add_block(vec![Operation::Swap], None).unwrap();
