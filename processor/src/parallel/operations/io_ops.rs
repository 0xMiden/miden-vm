...
impl CoreTraceFragmentGenerator {
    ...
    pub(crate) fn op_advpop(&mut self) {
        let value = self.context.replay.advice.replay_pop_stack();
    ...
    pub(crate) fn op_advpopw(&mut self) {
        let word = self.context.replay.advice.replay_pop_stack_word();
    ...
    pub(crate) fn op_mloadw(&mut self) {
        ...
        let word = self.context.replay.memory.replay_read_word(addr);
        self.stack_write_word(0, &word);
    }
    ...
    pub(crate) fn op_mload(&mut self) {
        let element = {
            let addr = self.stack_get(0);
            self.context.replay.memory.replay_read_element(addr)
        };
    ...
    pub(crate) fn op_mstream(&mut self) {
        ...
        let words = [
            self.context.replay.memory.replay_read_word(addr_first_word),
            self.context.replay.memory.replay_read_word(addr_second_word),
        ];
    ...
    pub(crate) fn op_pipe(&mut self) {
        ...
        let words = self.context.replay.advice.replay_pop_stack_dword();
