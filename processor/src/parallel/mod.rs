...
use crate::{
    ...
    fast::{
        ...
        trace_state::{
            ...
            DecoderState, ExecutionContextSystemInfo, HasherOp, HasherReplay,
            HasherReplayForChiplet, KernelReplay, MemoryReadsReplay, MemoryWritesReplay,
...
fn initialize_chiplets(
    ...
    hasher_for_chiplet: HasherReplayForChiplet,
    ...
    // the trace.
    {
        let elements_written: Box<dyn Iterator<Item = MemoryAccess>> =
            Box::new(memory_writes.elements_written.iter().map(|(addr, element, ctx, clk)| {
        ...
        let words_written: Box<dyn Iterator<Item = MemoryAccess>> = Box::new(
            memory_writes
                .words_written
                .iter()
                .map(|(addr, word, ctx, clk)| MemoryAccess::WriteWord(*addr, *word, *ctx, *clk)),
        );
        let elements_read: Box<dyn Iterator<Item = MemoryAccess>> =
            Box::new(core_trace_contexts.iter().flat_map(|ctx| {
                ctx.replay
                    .memory_reads
                    .iter_read_elements()
                    .map(|(addr, _, ctx, clk)| MemoryAccess::ReadElement(addr, ctx, clk))
            }));
        let words_read: Box<dyn Iterator<Item = MemoryAccess>> =
            Box::new(core_trace_contexts.iter().flat_map(|ctx| {
                ctx.replay
                    .memory_reads
                    .iter_read_words()
                    .map(|(addr, _, ctx, clk)| MemoryAccess::ReadWord(addr, ctx, clk))
            }));
