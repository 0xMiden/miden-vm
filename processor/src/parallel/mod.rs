...
use miden_air::{
    ...
    trace::{
        ...
        FMP_COL_IDX, FN_HASH_RANGE, IN_SYSCALL_COL_IDX, MIN_TRACE_LEN, RANGE_CHECK_TRACE_RANGE,
...
use miden_core::{
    Felt, ONE, OPCODE_PUSH, Operation, QuadFelt, StarkField, WORD_SIZE, Word, ZERO,
...
use crate::{
    ColMatrix, ContextId, ExecutionError, ProcessState,
...
fn fixup_stack_and_system_rows(
    ...
    {
        let last_fragment = fragments.last_mut().expect("expected at least one trace fragment");
        let last_system_state = system_rows.last().expect(
            "system_rows should not be empty, which indicates that there are no trace fragments",
        );
        let last_stack_state = stack_rows.last().expect(
...
fn pad_trace_columns(
    ...
    for row_idx in 0..trace_len {
        ...
        for col_idx in RANGE_CHECK_TRACE_RANGE {
...
/// initialize the `SpanContext` correctly to reflect the state of the decoder at that point. This
/// function does that initialization.
///
/// Recall that `SpanContext` keeps track of the state needed to correctly fill in the decoder
...
/// the current operation within that batch, and initializes the `SpanContext` accordingly. In other
/// words, it figures out how many operations are left in the current operation group, and how many
/// operation groups are left in the basic block, given that we are starting execution from the
/// specified operation.
...
impl StackInterface for CoreTraceFragmentGenerator {
    ...
    fn increment_size(&mut self, _tracer: &mut impl Tracer) {
...
impl Processor for CoreTraceFragmentGenerator {
    ...
    fn op_eval_circuit(
        &mut self,
        _err_ctx: &impl crate::ErrorContext,
