...
use crate::{
    ...
    fast::checkpoints::CoreTraceState,
    processor::Processor,
    ...
    fn execute_fragment_generation(
        ...
        match execution_phase {
            ...
            NodeExecutionPhase::End => {
                ...
                // Pop the continuation and add the appropriate end trace row
                // TODO(plafer): Do we want to pop continuation here? Do we need
                // `NodeExecutionPhase::End` anymore?
                let continuation = self
                    .state
                    .traversal
                    .pop_continuation()
                    .expect("Expected continuation for END phase");

                match continuation {
                    Continuation::FinishJoin(node_id) => {
                        let mast_node = program.get_node_by_id(node_id).expect("node should exist");
                        if let MastNode::Join(join_node) = mast_node {
                    ...
                    Continuation::FinishSplit(node_id) => {
                        let mast_node = program.get_node_by_id(node_id).expect("node should exist");
                        if let MastNode::Split(split_node) = mast_node {
                    ...
                    Continuation::FinishLoop(node_id) => {
                        let mast_node = program.get_node_by_id(node_id).expect("node should exist");
                        if let MastNode::Loop(loop_node) = mast_node {
                    ...
                    Continuation::FinishCall(node_id) => {
                        let mast_node = program.get_node_by_id(node_id).expect("node should exist");
                        if let MastNode::Call(call_node) = mast_node {
                    ...
                    Continuation::FinishDyn(node_id) => {
                        let mast_node = program.get_node_by_id(node_id).expect("node should exist");
                        if let MastNode::Dyn(dyn_node) = mast_node {
                            self.add_dyn_end_trace_row(dyn_node)?;
                        } else {
                            panic!("Expected dyn node");
                        }
                    },
                    other => panic!("Unexpected continuation in END phase: {:?}", other),
                }
            },
        }

        // Main execution loop similar to FastProcessor::execute_impl
        let mut current_forest = Arc::new(program.clone());
