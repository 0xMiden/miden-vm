...
use crate::{
    ...
    fast::{
        ...
        checkpoints::{
            CoreTraceState, DecoderState, ExecutionContextSystemInfo, NodeExecutionPhase, NodeFlags,
        },
    },
    processor::Processor,
...
fn fixup_stack_and_system_rows(
    ...
    // TODO(plafer): cleanup
    // Populate the last row into the last fragment
    {
        let last_fragment = fragments.last_mut().unwrap();
        let last_system_state = system_rows.last().unwrap();
        let last_stack_state = stack_rows.last().unwrap();
        ...
        for i in 0..NUM_OP_BITS {
            ...
            last_fragment.columns[DECODER_TRACE_OFFSET + OP_BITS_OFFSET + i].push(bit_value);
        }
...
fn combine_fragments(fragments: Vec<CoreTraceFragment>, program_hash: Word) -> MainTrace {
    ...
    // Pad the trace length to the next power of two and ensure that there is space for random rows
    // (or at least the minimum trace length). Note that `total_program_rows` contains one HALT
    // operation, which is considered "padding" when considering random rows.
    let trace_len =
        core::cmp::max((total_program_rows + NUM_RAND_ROWS - 1).next_power_of_two(), MIN_TRACE_LEN);
...
impl CoreTraceFragmentGenerator {
    ...
    fn execute_fragment_generation(
        ...
        match execution_phase {
            NodeExecutionPhase::BasicBlock { node_id, batch_index, op_idx_in_batch } => {
            ...
            NodeExecutionPhase::Respan { node_id, batch_index } => {
                let basic_block_node = {
                    let mast_node =
                        initial_mast_forest.get_node_by_id(node_id).expect("node should exist");
                    mast_node.get_basic_block().expect("Expected a basic block node")
                ...
                self.span_context = {
                    ...
                    let num_groups_left: usize = basic_block_node
                        ...
                        .iter()
                        .skip(batch_index)
                        .map(|batch| batch.num_groups().next_power_of_two())
                        .sum();
            ...
            NodeExecutionPhase::LoopRepeat(node_id) => {
                ...
                while condition == ONE {
                    self.add_loop_repeat_trace_row(
                        loop_node,
                        &initial_mast_forest,
                        self.state.decoder.current_addr,
                    )?;
            ...
            NodeExecutionPhase::End(node_id) => {
                ...
                match mast_node {
                    ...
                    MastNode::Call(call_node) => {
                        let ctx_info = self.state.block_stack_replay.replay_execution_context();
        ...
        match mast_node {
            MastNode::Block(basic_block_node) => {
                ...
                self.add_span_start_trace_row(
                    ...
                    self.state.decoder.parent_addr,
                )?;
            ...
            MastNode::Join(join_node) => {
                ...
                self.add_join_start_trace_row(join_node, program, self.state.decoder.parent_addr)?;
            ...
            MastNode::Loop(loop_node) => {
                ...
                self.add_loop_start_trace_row(loop_node, program, self.state.decoder.parent_addr)?;
            ...
            MastNode::Call(call_node) => {
                ...
                self.state.stack.start_context();
                ...
                if call_node.is_syscall() {
                    ...
                    self.state.system.ctx = ContextId::from(self.state.system.clk + 1); // New context ID
                    self.state.system.fmp = Felt::new(FMP_MIN);
                    self.state.system.fn_hash = program[call_node.callee()].digest();
                }

                // Add "start CALL/SYSCALL" row
                self.add_call_start_trace_row(call_node, program, self.state.decoder.parent_addr)?;
            ...
            MastNode::Dyn(dyn_node) => {
                ...
                let callee_hash = {
                    let mem_addr = self.state.stack.get(0);
                    self.state.memory.replay_read_word(mem_addr)
                };
                ...
                if dyn_node.is_dyncall() {
                    ...
                    self.state.system.ctx = ContextId::from(self.state.system.clk + 1); // New context ID
                    self.state.system.fmp = Felt::new(FMP_MIN);
                    self.state.system.fn_hash = callee_hash;
                ...
                match program.find_procedure_root(callee_hash) {
                    ...
                    // TODO(plafer): the fast processor needs to record the MAST Forest resolution
                    // in this case too
                    None => todo!(),
                };
    ...
    // TODO(plafer): clean this up; it's probably not clear from the `CoreTraceState` that this is
    // the relationship between the replays and the `DecoderState` (only made clear when looking at
    // this function)
    fn update_decoder_state_on_node_start(&mut self) {
        let DecoderState { current_addr, parent_addr } = &mut self.state.decoder;

        *current_addr = self.state.hasher.replay_block_address();
        *parent_addr = self.state.block_stack_replay.replay_node_start();
    }

    fn update_decoder_state_on_node_end(&mut self) -> (Felt, NodeFlags) {
        let DecoderState { current_addr, parent_addr } = &mut self.state.decoder;

        let node_end_data = self.state.block_stack_replay.replay_node_end();
    ...
    fn restore_context_from_replay(&mut self, ctx_info: &ExecutionContextSystemInfo) {
        self.state.system.ctx = ctx_info.parent_ctx;
        self.state.system.fmp = ctx_info.parent_fmp;
        self.state.system.fn_hash = ctx_info.parent_fn_hash;
        self.state.system.in_syscall = false;

        self.state.stack.restore_context(&mut self.state.stack_overflow);
    }
    ...
    fn execute_op_batch(
        ...
        let (static_ops_to_skip, execute_noop_first) = if start_op_idx > 0 {
            let ops = batch.ops();
            let mut runtime_op_idx = 0; // Tracks the actual runtime operation index (including NOOPs)
            let mut static_ops_to_skip = 0; // Tracks the index in the static operations array
            ...
            // to skip
            while static_ops_to_skip < ops.len() && runtime_op_idx < start_op_idx {
                let op = ops[static_ops_to_skip];
                ...
                runtime_op_idx += 1;
                static_ops_to_skip += 1;
            }

            (static_ops_to_skip, execute_noop_first)
        } else {
            (0, false)
        };
        ...
        if execute_noop_first {
            ...
            // TODO(plafer): I think this should be `num_batch_groups - 1` (see
            // Process::execute_op_batch). Same for other call to `start_op_group()`
    ...
    fn dispatch_operation(&mut self, op: &Operation) -> Option<[Felt; NUM_USER_OP_HELPERS]> {
        ...
        match op {
            ...
            Operation::Emit(event_id) => {
                user_op_helpers = Some([Felt::from(*event_id), ZERO, ZERO, ZERO, ZERO, ZERO]);
            },
            ...
            Operation::Expacc => {
                let expacc_helpers = self.op_expacc();
            ...
            Operation::U32assert2(err_code) => {
                let u32assert2_helpers = self.op_u32assert2(*err_code);
...
fn initialize_span_context(
    ...
    let num_groups_left = {
        ...
        // account for the current group (since `num_groups_left` is the number of groups left *when
        // done* the current group)
    ...
    while static_op_idx < ops.len() {
        ...
        if op_idx_in_group == op_counts[group_idx] - 1 {
            ...
            if has_imm {
                ...
                if runtime_op_idx == op_idx_in_batch {
                    return (group_idx, op_idx_in_group + 1);
                }
                runtime_op_idx += 1; // Account for the inserted NOOP
            }
...
impl Processor for CoreTraceFragmentGenerator {
    ...
    fn increment_stack_size(&mut self) {
        ...
        // push the last element on the overflow table
        {
            ...
            self.state.stack.push_overflow(last_element, self.clk());
        }
    ...
    fn decrement_stack_size(&mut self) {
        ...
        if let Some(last_element) = self.state.stack.pop_overflow(&mut self.state.stack_overflow) {
