...
use crate::{
    ...
    decoder::{
        SpanContext,
        block_stack::{BlockInfo, BlockType, ExecutionContextInfo},
    },
    fast::{
...
impl CoreTraceFragmentGenerator {
            ...
            NodeExecutionPhase::End(node_id) => {
                ...
                match mast_node {
                    ...
                    MastNode::Loop(loop_node) => {
                        ...
                        let block_info = self.state.block_stack.peek();
                        if block_info.is_entered_loop() == ONE {
                            self.decrement_stack_size();
                        }
                    ...
                    MastNode::Call(call_node) => {
                        ...
                        self.restore_context(&block_info);
                        self.add_call_end_trace_row(call_node, &initial_mast_forest, block_info)?;
                    },
                    MastNode::Dyn(dyn_node) => {
                        ...
                        self.add_dyn_end_trace_row(dyn_node, block_info)?;
                    },
        ...
        while let Some(continuation) = self.state.traversal.pop_continuation() {
            match continuation {
                ...
                Continuation::FinishLoop(node_id) => {
                    ...
                    // Note that we don't confirm that the condition is properly ZERO here, as
                    // the FastProcessor already ran that check.
                    self.add_loop_end_trace_row(loop_node, &current_forest)?;
                },
                Continuation::FinishCall(node_id) => {
                    ...
                    self.add_call_end_trace_row(
                        mast_node.unwrap_call(),
                        &current_forest,
                        block_info,
                    )?;
                },
    ...
    fn execute_mast_node(&mut self, node_id: MastNodeId, program: &MastForest) -> ControlFlow<()> {
        ...
        match mast_node {
            ...
            MastNode::Call(call_node) => {
                ...
                self.add_call_end_trace_row(call_node, program, block_info)
            },
            MastNode::Dyn(dyn_node) => {
                let callee_hash = {
                ...
                if dyn_node.is_dyncall() {
                    ...
                    let parent_addr = self.state.block_stack.push(addr, BlockType::Dyn, None);

                    // Pop the memory address off the stack, and write the DYN trace row
                    self.decrement_stack_size();
                    self.add_dyn_start_trace_row(dyn_node, parent_addr, callee_hash, None)?;
                };
    ...
    fn restore_context(&mut self, block_info: &BlockInfo) {
        let ctx_info = block_info
            .ctx_info
            .expect("expected execution context info when restoring context");
        ...
        self.state.stack.restore_context();
    }
