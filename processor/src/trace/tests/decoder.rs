...
use crate::{ContextId, decoder::{BlockHashTableRow, build_op_group}};
use miden_core::mast::{BasicBlockNodeBuilder, JoinNodeBuilder};
...
fn decoder_p2_join() {
    ...
    let basic_block_1 = BasicBlockNodeBuilder::new(vec![Operation::Mul], Vec::new()).build().unwrap();
    let basic_block_1_id = mast_forest.add_node(basic_block_1.clone()).unwrap();

    let basic_block_2 = BasicBlockNodeBuilder::new(vec![Operation::Add], Vec::new()).build().unwrap();
    let basic_block_2_id = mast_forest.add_node(basic_block_2.clone()).unwrap();

    let join = JoinNodeBuilder::new([basic_block_1_id, basic_block_2_id]).build(&mast_forest).unwrap();
...
fn decoder_p2_loop_with_repeat() {
    ...
    let basic_block_1 = BasicBlockNodeBuilder::new(vec![Operation::Pad], Vec::new()).build().unwrap();
    let basic_block_1_id = mast_forest.add_node(basic_block_1.clone()).unwrap();

    let basic_block_2 = BasicBlockNodeBuilder::new(vec![Operation::Drop], Vec::new()).build().unwrap();
    let basic_block_2_id = mast_forest.add_node(basic_block_2.clone()).unwrap();
