...
fn b_chip_trace_mem() {
    ...
    // is: 2 hasher controller rows + 30 padding + 32 hasher permutation segment = 64 rows. No bitwise ops, so
    // memory starts at row 64.
    let memory_start = 2 * HASH_CYCLE_LEN; // controller(32 padded) + perm(32)
