...
impl CircuitEvaluation {
    ...
    ///
    /// Returns an error if `num_read_rows` and `num_eval_rows` together require more than
    /// [`MAX_NUM_ACE_WIRES`] wires, preventing an arithmetic overflow and ensuring that
    /// both debug and release builds behave identically when inputs are out of range.
    ///
    /// Previously this function used `assert!()` which caused a panic in debug builds
    /// but silently continued with an incorrect `num_wires` value in release builds due
    /// to u32 wrapping (fixes #1820, reported by FuzzingLabs).
    pub fn new(
        ctx: ContextId,
        clk: RowIndex,
        num_read_rows: u32,
        ...
        if num_wires > MAX_NUM_ACE_WIRES as u64 {
            return Err(AceError(format!(
                "circuit requires {num_wires} wires but the maximum is {MAX_NUM_ACE_WIRES}"
            ))
            .into());
        }

        // num_rows <= num_wires <= MAX_NUM_ACE_WIRES < 2^30, so this addition is safe.
