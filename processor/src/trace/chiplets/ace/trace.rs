...
/// The processor checks that the final output is zero before recording the evaluation.
///
...
impl CircuitEvaluation {
    ...
    ///
    /// # Panics
    /// Panics if the total wire count is zero or exceeds [`MAX_NUM_ACE_WIRES`], or a vector
    /// reservation fails.
    pub fn new(ctx: ContextId, clk: RowIndex, num_read_rows: u32, num_eval_rows: u32) -> Self {
        Self::try_new(ctx, clk, num_read_rows, num_eval_rows)
            .expect("invalid ACE dimensions or failed reservation")
    }

    /// Allocates circuit storage using fallible reservations. The calling processor is responsible
    /// for enforcing execution resource limits before construction.
    pub(crate) fn try_new(
        ...
        if num_wires == 0 {
            return Err(AceError("ACE circuit must contain at least one wire".into()));
        }
        if num_wires > MAX_NUM_ACE_WIRES as u64 {
            return Err(AceError("too many ACE wires".into()));
        }
        let mut wires = Vec::new();
        wires
            .try_reserve_exact(num_wires as usize)
            .map_err(|_| AceError("failed to reserve ACE wires".into()))?;
        let mut read_nodes = Vec::new();
        read_nodes
            .try_reserve_exact(num_read_rows as usize)
            .map_err(|_| AceError("failed to reserve ACE read nodes".into()))?;
        let mut eval_nodes = Vec::new();
        eval_nodes
            .try_reserve_exact(num_eval_rows as usize)
            .map_err(|_| AceError("failed to reserve ACE evaluation nodes".into()))?;
        ...
        Ok(Self {
            ...
            wire_bus: WireBus {
                ...
                id_next: Felt::from_u32(num_wires as u32 - 1),
            },
            read_nodes,
            eval_nodes,
        })
