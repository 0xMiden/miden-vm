...
/// Generates core trace columns in parallel using a flat row-major buffer for cache-friendly
/// writes, then transposes to column-major format for compatibility with the rest of the pipeline.
fn generate_core_trace_columns(
    ...
    // Allocate a flat row-major buffer: data[row * CORE_TRACE_WIDTH + col]
    ...
    // Split flat buffer into per-fragment RowMajorTraceWriter slices
    ...
    // Collect stack_rows, system_rows, and total row count
    ...
    // Fix up stack and system rows (first row of each fragment)
    ...
    // Truncate the flat buffer to the actual number of rows written
    core_trace_data.truncate(total_core_trace_rows * CORE_TRACE_WIDTH);

    // Append the HALT opcode row
    ...
    let matrix = RowMajorMatrix::new(core_trace_data, CORE_TRACE_WIDTH);
    let transposed = matrix.transpose();
    let core_trace_columns: Vec<Vec<Felt>> =
        transposed.row_slices().map(|row| row.to_vec()).collect();
...
fn fixup_stack_and_system_rows(
    ...
    // Initialize the first fragment's first row with system state and initial stack
    {
        let row = &mut core_trace_data[..w];

        // System state: all zeros for the first row
        // (row is already zeroed from allocation, but be explicit for system columns)
    ...
    // Determine the number of fragments and initialize subsequent fragments
...
/// Appends a row with the HALT opcode to the end of the flat row-major buffer.
///
/// This ensures that the trace ends with at least one HALT operation, which is necessary to satisfy
/// the constraints. `num_rows_before` is the current row count before the HALT row is appended.
fn push_halt_opcode_row(
    ...
    // First 4 hasher columns: copy the last value to propagate program hash
    ...
    // Remaining 4 hasher columns: ZEROs (already)

    // in_span, group_count, op_index, op_batch_flags: all ZEROs (already)

    // op_bit_extra columns:
    // First column = ZERO (already), second column = ONE (product of two MSBs of HALT, both 1)
