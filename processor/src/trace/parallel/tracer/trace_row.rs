...
impl<'a> CoreTraceGenerationTracer<'a> {
    /// Fills a complete trace row by assembling system, decoder, and stack columns into a
    /// contiguous row buffer and writing it to the row-major trace writer in one shot.
    ///
    /// System and stack columns are one row behind (buffered from the previous call).
    /// Decoder columns are written immediately from the current `DecoderRow`.
    fn fill_trace_row(
        &mut self,
        system: &SystemState,
        stack: &StackState,
        ...
        // Buffer current system/stack state for the next row
    ...
    /// Writes decoder columns into the row buffer at their correct offsets.
    fn write_decoder_to_row(row: &mut [Felt; CORE_TRACE_WIDTH], decoder_row: &DecoderRow) {
        ...
        // Extra bit columns — computed directly from opcode (no read-back needed)
    ...
    /// Builds the buffered system trace column values for the next row.
    ...
    /// Builds the buffered stack trace column values for the next row.
