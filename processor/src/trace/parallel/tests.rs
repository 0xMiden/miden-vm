...
fn test_trace_generation_at_fragment_boundaries(
    ...
    for (label, air_frag, air_single) in [
        ...
        let (aux_frag, committed_frag, aux_single, committed_single) = if label == "Core" {
            let (a, c) = build_logup_aux_trace(&CoreAir, air_frag, &challenges);
            let (b, d) = build_logup_aux_trace(&CoreAir, air_single, &challenges);
            (a, c, b, d)
        } else {
            let (a, c) = build_logup_aux_trace(&ChipletsAir, air_frag, &challenges);
            let (b, d) = build_logup_aux_trace(&ChipletsAir, air_single, &challenges);
            (a, c, b, d)
        };
        assert_eq!(
