...
fn test_trace_generation_at_fragment_boundaries(
    ...
    // Verify deferred state matches deterministically.
    assert_eq!(
        trace_from_fragments.deferred_state().root(),
        trace_from_single_fragment.deferred_state().root(),
    );
    assert_eq!(
        trace_from_fragments.deferred_state().to_wire().unwrap(),
        trace_from_single_fragment.deferred_state().to_wire().unwrap(),
    );
...
///
/// `ExecutionTrace` can contain internal structures whose default debug output may drift. This
/// wrapper limits output to deterministic trace-level fields for snapshot testing.
