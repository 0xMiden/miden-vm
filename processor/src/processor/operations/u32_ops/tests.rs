...
use miden_core::{Felt, PrimeCharacteristicRing, ZERO, stack::MIN_STACK_DEPTH};
...
fn test_op_u32assert2_both_invalid() {
    ...
    let result = op_u32assert2(&mut processor, Felt::from_u32(123), &(), &mut tracer);
...
fn test_op_u32assert2_second_invalid() {
    ...
    let result = op_u32assert2(&mut processor, Felt::from_u32(456), &(), &mut tracer);
    assert!(result.is_err());
}
