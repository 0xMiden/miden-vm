...
pub enum OperationHelperRegisters {
    ...
    /// Helper for the `LOG_DEFERRED` operation, which folds the per-call statement digest into
    /// the rolling deferred root via a Poseidon2 permutation.
    ///
    /// - `addr`: the address in the hasher chiplet where the permutation is recorded.
    /// - `prev_deferred_root`: the previous deferred root word, used as the rate0 input to the
    ///   permutation.
    LogDeferred { addr: Felt, prev_deferred_root: Word },
...
impl OperationHelperRegisters {
    pub fn to_user_op_helpers(&self) -> [Felt; NUM_USER_OP_HELPERS] {
        match self {
            ...
            Self::LogDeferred { addr, prev_deferred_root } => [
                *addr,
                prev_deferred_root[0],
                prev_deferred_root[1],
                prev_deferred_root[2],
                prev_deferred_root[3],
                ZERO,
            ],
