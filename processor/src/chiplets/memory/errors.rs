use alloc::string::String;

use miden_utils_diagnostics::{Diagnostic, miette};

use crate::{ContextId, errors::ExecErrorTrait};

#[derive(Debug, thiserror::Error, Diagnostic)]
pub enum MemoryError {
    #[error("memory address cannot exceed 2^32 but was {0}")]
    AddressOutOfBounds(u64),
    #[error(
        "memory address {addr} in context {ctx} was read and written, or written twice, in the same clock cycle"
    )]
    IllegalMemoryAccess { ctx: ContextId, addr: u32 },
    #[error(
        "memory range start address cannot exceed end address, but was ({start_addr}, {end_addr})"
    )]
    InvalidMemoryRange { start_addr: u64, end_addr: u64 },
    #[error("word memory access at address {addr} in context {ctx} is unaligned")]
    #[diagnostic(help(
        "ensure that the memory address accessed is aligned to a word boundary (it is a multiple of 4)"
    ))]
    UnalignedWordAccess { addr: u32, ctx: ContextId },
}

impl ExecErrorTrait for MemoryError {
    fn custom_label(&self) -> Option<String> {
        match self {
            Self::UnalignedWordAccess { addr, .. } => {
                Some(format!("tried to access memory address {addr}"))
            },
            Self::AddressOutOfBounds { .. } => None, // Uses default label
            Self::IllegalMemoryAccess { .. } => None, // No label needed
            Self::InvalidMemoryRange { .. } => None, // No label needed
        }
    }
}
