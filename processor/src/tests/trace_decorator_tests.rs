...
fn create_trace_test_program() -> Program {
...
/// Test that verifies trace decorators only execute when tracing is enabled
#[test]
fn test_trace_decorators_only_execute_when_tracing_enabled() {
    ...
    // Test with tracing OFF - decorator should NOT execute
    let mut host_tracing_off = TestHost::new();
    let process_tracing_off = FastProcessor::new(StackInputs::default());

    let result = process_tracing_off.execute_sync(&program, &mut host_tracing_off);
    assert!(result.is_ok(), "Execution failed: {result:?}");
    assert!(
        host_tracing_off.get_trace_count(999) == 0,
        "Decorator should NOT execute when tracing is OFF"
    );

    // Test with tracing ON - decorator should execute
    let mut host_tracing_on = TestHost::new();
    let process_tracing_on = FastProcessor::new(StackInputs::default())
        .with_advice(AdviceInputs::default())
        .expect("advice inputs should fit advice map limits")
        .with_tracing(true);

    let result = process_tracing_on.execute_sync(&program, &mut host_tracing_on);
    assert!(result.is_ok(), "Execution failed: {result:?}");
    assert!(
        host_tracing_on.get_trace_count(999) == 1,
        "Decorator SHOULD execute when tracing is ON"
    );
}

/// Test that verifies trace decorators do NOT execute when tracing is OFF
#[test]
fn test_trace_decorators_only_execute_when_tracing_off() {
    ...
    // Create process with tracing OFF
    ...
    assert_eq!(host.get_trace_count(999), 0, "Decorator should NOT execute when tracing is OFF");

    // Verify no execution events were recorded
    let order = host.get_execution_order();
    assert_eq!(order.len(), 0, "No trace events should occur when tracing is OFF");
}

/// Test that verifies trace decorators DO execute when tracing is ON
#[test]
fn test_trace_decorators_only_execute_when_tracing_on() {
    // Create a test program with a Trace decorator
    let program = create_trace_test_program();
    ...
    // Create processor with tracing ON
    let processor = FastProcessor::new(StackInputs::default())
        .with_advice(AdviceInputs::default())
        .expect("advice inputs should fit advice map limits")
        .with_tracing(true);
    ...
    assert_eq!(
        ...
        1,
        "Decorator should execute exactly once when tracing is ON"
    );

    // Verify execution event was recorded
    let order = host.get_execution_order();
    assert_eq!(order.len(), 1, "Should have exactly 1 trace event when tracing is ON");
...
/// with tracing on vs off for a more complex program
#[test]
fn test_zero_overhead_when_tracing_off() {
