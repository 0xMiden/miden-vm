...
- [BREAKING] `midenc_hir_type` now supports self-recursive and mutually recursive struct and enum types, e.g. `struct Node { next: *Node }`. Recursion must cross a pointer, list, or function, so that every layout stays finite; more precisely, every cycle in the type reference graph must cross one of those. A recursive `Type` carries its whole definition group by `Arc`, so it remains self-contained: no interning table, definition registry, or context object is needed to interpret one, and descending through a backedge yields a type equal to the definition it points at.
- `midenc_hir_type::RecursiveTypeBuilder` keys definitions by a binding key that is distinct from the aggregate's declared name, so routing a declaration through the builder never renames it. Use `StructTemplate::named` to give a struct its own name; `RecTypeRef::key` returns the binding key and `RecTypeRef::name` the declared name.
- [BREAKING] `midenc_hir_type::Type::Struct` and `Type::Enum` now carry `StructRef` and `EnumRef` rather than `Arc<StructType>` and `Arc<EnumType>`. Pattern matching on `Type::Struct(..)`/`Type::Enum(..)` is unchanged and correctly selects recursive types too, but reading an aggregate's contents now goes through `StructRef::get`/`EnumRef::get`, which return `Cow` and unfold the recursive case. `Type` grows from 16 to 24 bytes as a result.
- Fixed a stack overflow when assembling a module containing a cyclic type declaration. `type A = B` with `type B = A`, or a type defined in terms of itself, previously aborted the process, because resolving a type reference re-entered declaration resolution with a fresh nesting budget. Such cycles are now reported as a `recursive type definition` diagnostic. Resolved type declarations are also cached now, where the cache was previously populated but never consulted.
- Recursive types are now representable in package debug info. Aggregate rows are reserved before their bodies are registered, so a backedge has an index to point at, and recovery partitions the reachable type graph into strongly connected components and rebuilds each recursive one through the same builder used elsewhere. Recovery therefore rejects a cycle that crosses no pointer, list, or function, and a cycle containing no aggregate at all. Recovering a type now starts from a type table index rather than a row, since a row alone cannot tell whether a child refers back to it. As with `TypeRepr`, recovery reproduces a recursive type's shape rather than its exact identity: a debug row carries one name, so it cannot hold both a definition's binding key and its declared name.
