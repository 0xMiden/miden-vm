...
- Deferred-DAG register events now derive the node digest in-circuit rather than returning it via advice: `sys::register_expr` computes it with `hperm` and `sys::register_chunk` with a `mem_stream` linear hash (the latter gains an `n_chunks` operand), binding the digest folded into the transcript to the operand-stack payload and to memory. `adv.evaluate_deferred` now returns the canonical's `tag || payload` on the advice stack (no advice-map entry, no digest push) and the `sys::evaluate` wrapper is removed — the raw event is wrapped per-precompile. Chunk nodes must carry at least one chunk (`NodeType::Chunks` holds a `NonZeroU32`); the empty `n == 0` case is forbidden, so host and in-circuit chunk hashing agree without a special case.
