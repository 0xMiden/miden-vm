...
- [BREAKING] Migrated all four precompiles (keccak256, sha512, ecdsa_k256_keccak, eddsa_ed25519) to per-app schema apps inside the deferred-DAG framework. Each app's `reduce` runs the real kernel (`Keccak256::hash`, `Sha512::hash`, `PublicKey::verify_prehash`, `PublicKey::verify_with_unchecked_k`) at evaluation time, and the verifier replays them via `DeferredState::rehydrate`. The legacy `PrecompileVerifier` / `PrecompileRequest` / `PrecompileCommitment` / `PrecompileVerifierRegistry` framework is deleted, along with the four per-precompile event handlers. Wire format for `ExecutionProof` changes: it no longer carries `Vec<PrecompileRequest>` (the deferred state is the precompile witness). MASM-side, the public `keccak256::hash_bytes` / `sha512::hash_bytes` / `ecdsa_k256_keccak::verify` / `eddsa_ed25519::verify` procedure names and stack contracts are unchanged, but `verify_prehash` for the two signature schemes now takes a single `buf_ptr` to a contiguous chunk buffer (was: three separate pk/digest/sig pointers). The deferred-DAG sys events get a redesigned API: `adv.register_deferred{,_chunk}` push the registered node's digest onto the advice stack, and `adv.evaluate_deferred` switches from `(tag, payload)` input to a `NODE_DIGEST` input. New MASM helpers in `sys/mod.masm`: `register_expr`, `register_chunk`, `evaluate`, `log_node_digest`. The old `log_precompile_request` wrapper is removed; the `log_precompile` opcode itself is unchanged ([#3172](https://github.com/0xMiden/miden-vm/pull/3172)).
