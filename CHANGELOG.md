...
- [BREAKING] Migrated all four precompiles (keccak256, sha512, ecdsa_k256_keccak, eddsa_ed25519) onto the deferred-DAG framework. Each precompile's `reduce` runs the real kernel (`Keccak256::hash`, `Sha512::hash`, `PublicKey::verify_prehash`, `PublicKey::verify_with_unchecked_k`) at evaluation time, and the verifier replays them via `DeferredState::rehydrate`. The legacy `PrecompileVerifier` / `PrecompileRequest` / `PrecompileCommitment` / `PrecompileVerifierRegistry` framework and the four per-precompile event handlers are deleted. `ExecutionProof` no longer carries `Vec<PrecompileRequest>` — it ships the deferred-DAG wire state (`DeferredStateWire`), which is the precompile witness. MASM-side, the public `keccak256::hash_bytes` / `sha512::hash_bytes` / `ecdsa_k256_keccak::verify` / `eddsa_ed25519::verify` procedure names and stack contracts are unchanged, except `verify_prehash` for the two signature schemes now takes a single `buf_ptr` to a contiguous chunk buffer (was: three separate pk/digest/sig pointers). The old `log_precompile_request` wrapper is removed; the `log_precompile` opcode itself is unchanged ([#3172](https://github.com/0xMiden/miden-vm/pull/3172)).

#### Fixes

- Bounded the deferred-DAG state accumulated during execution via a new `ExecutionOptions::max_deferred_elements` cap (default 2^20 field elements), restoring the host-memory backstop the deferred-DAG migration dropped when it removed the per-invocation `max_hash_len_bytes` limit. Enforced in the `DeferredRegister{,Chunk}` / `DeferredEvaluate` system-event handlers, with the chunk handler checking the projected size before reading memory.
