...
- [BREAKING] Import syntax in MASM now distinguishes module imports from item imports. Module imports use `use some::module` or `use some::module as alias` and may not have `pub` visibility. Item imports use `use {item} from some::module` or `use {item1, item2 as alias} from some::module` and may have `pub` visibility. See ([#3220](https://github.com/0xMiden/miden-vm/pull/3220)).
...
- [BREAKING] Imports in MASM may no longer refer to other imports in scope. Imports are now resolved in the global namespace (i.e. as if the path is absolute). Imports relative to a submodule must now use an explicit `self::` prefix. See ([#3220](https://github.com/0xMiden/miden-vm/pull/3220)).
