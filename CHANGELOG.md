...
- [BREAKING] Added a matrix finalizer to Eidos compression that outputs four full-range Goldilocks elements, changing every Eidos digest, root, and proof artifact. `HashFunction::Eidos.collision_resistance()` returns 128.
- Counted the challenge field as 128 bits in the native and recursive security estimators. Reported levels for supported configurations are unchanged.
