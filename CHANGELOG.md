...
- [BREAKING] Bound cumulative ACE witness capacity and count-dependent work during execution. The limit defaults to 64 MiB and can be raised with `ExecutionOptions::with_max_ace_witness_bytes` or the CLI's `--max-ace-witness` option; the CLI also caps it at `--max-prover-memory` ([#3904](https://github.com/0xMiden/miden-vm/pull/3904)).
