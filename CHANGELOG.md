...
#### Changes
- Added content-addressed proof requests: `miden_core::program::request_key(verifier_root, claim_commitment)` and its MASM mirror `sys::vm::claim::request_key` derive the advice-map key `H(claim_commitment ‖ verifier_root)` under a registered request domain, letting a consumer fetch a proof package by content instead of by advice-stack position.
- Added the recursive-verification usage layer: `miden_verifier::recursive` builds the advice the MASM verifier consumes, and the new `sys::vm::verify_vm_proof_from_claim` entrypoint derives the claim commitment from the caller-staged claim, fetches its proof from the advice map under `H(verifier_root, claim_commitment)`, and verifies it.
- [BREAKING] Reworked the MASM verifier entrypoint around the caller-supplied claim region: `exec.vm::verify_proof` is now `exec.vm::verify_vm_proof` with inputs `[claim_ptr, kernel_ptr, num_kernel_digests]` and output `[D]` (the deferred obligation, previously absorbed and dropped). The verifier computes the kernel commitment into the claim region via the new `sys::vm::claim::kernel_commitment` export and binds the transcript with `sys::vm::claim::claim_commitment`; the separate stack-I/O operand region is gone (the claim region carries I/O).
- [BREAKING] Reworked the native verification API around the execution claim and linear obligations: `verify(proof, claim)` verifies fully settled packages only; `verify_unsettled(proof, claim)` returns the deferred obligation as a `#[must_use]` `Unsettled` token consumable by `settle(pending, evidence, budget)`. `ExecutionProof` now carries the statement-bound `deferred_root` plus optional settlement evidence, replacing the always-present wire; the old `verify(program_info, inputs, outputs, proof)` and `verify_with_max_deferred_elements` entrypoints are removed.
- [BREAKING] Collapsed the statement observation of the Fiat-Shamir transcript into one block `[CLAIM_HASH | deferred_root]`: with `RELATION_DIGEST` pre-loaded in the transcript capacity, the transcript realizes the factored statement binding `H(RELATION_DIGEST ‖ CLAIM_HASH ‖ D)`. The raw stack I/O and kernel digests remain public values for constraint evaluation but are no longer separately absorbed.
- [BREAKING] Added the canonical execution-claim schema: `ExecutionClaim` with a domain-tagged commitment (`CLAIM_HASH`), mirrored in MASM by `sys::vm::claim::claim_commitment`. Domain tags are registered numeric selectors (`miden_core::program::domain`), and kernel commitments are now domain-tagged, which changes their values. Adds `hash_elements_in_domain` to the Poseidon2 MASM module.
