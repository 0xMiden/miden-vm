...
- [BREAKING] Changed Eidos compression to output four full-range Goldilocks elements instead of 63-bit masked values, which changes every Eidos digest, root, and proof artifact. Removed `eidos::encoding::ODD_LANE_MASK` and the mask-specific Eidos AIR helpers; `pack_output_felt` and `output_cv_to_word` now panic on noncanonical lane pairs, and `HashFunction::Eidos.collision_resistance()` returns 128 instead of 126.
- Counted the challenge field as 128 bits in the native and recursive security estimators, raising the report cap from 127 to 128 bits. Reported levels for supported configurations are unchanged.
- Sped up native Eidos compression on aarch64 (NEON) and x86 (AVX2, AVX-512, and AVX-512 IFMA).
