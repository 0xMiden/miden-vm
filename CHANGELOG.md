...
#### Changes
- [BREAKING] Reworked recursive verification around canonical execution claims. `ExecutionClaim` (program digest, kernel commitment, stack i/o) carries a domain-tagged commitment that binds the Fiat-Shamir statement as `H(RELATION_DIGEST ‖ CLAIM_HASH ‖ D)`. Native verification becomes `Verifier::verify(proof, claim)` and `Verifier::verify_partial(proof, claim)`, which returns the deferred obligation as a `#[must_use]` `Unsettled` token. The MASM entrypoint becomes `exec.vm::verify_vm_proof` with inputs `[claim_ptr, kernel_ptr, num_kernel_digests]` and output `[D]`, `exec.vm::verify_vm_proof_from_claim` fetches a proof from the advice map by content under `request_key(verifier_root, claim_commitment)`, and `miden_verifier::recursive` builds the advice the MASM verifier consumes ([#3422](https://github.com/0xMiden/miden-vm/pull/3422)).
