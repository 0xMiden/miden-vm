...
#### Changes
- [BREAKING] Factored recursive ACE circuits into per-order and shared sections, generalized the registry infrastructure to arbitrary AIR sets, and added the 10-AIR precompile VM registry. This changes the Miden VM and precompile VM ACE roots, relation digests, circuit shapes, and recursive-proof transcripts.
...
- [BREAKING] Factored recursive ACE circuits into a per-order prefix and an order-invariant common section, with registry leaves `merge(H(prefix), H(common))`. The Miden VM registry now uses constant domain-separated padding leaves and serves one authenticated path through `config::ace_registry_path(tag)` instead of exposing a checked-in leaf array. This changes the generated MASM circuit, registry root, relation digest, and recursive-proof transcript.
- [BREAKING] Added the PVM ACE registry: one circuit per ordering of the ten precompile chiplets (`10!` active leaves in a depth-22 tree), committed by a generated root, relation digest, and depth-12 node row. The new release-mode `pvm-registry-regen` tool exhaustively rebuilds and cross-checks every ordering; PVM proving and verification now use the registry-derived relation digest.
- Generalized `miden-ace-codegen` multi-AIR construction and registry support over arbitrary AIR sets, including preprocessed traces, Lehmer order tags, subtree recomputation, authenticated node rows, and SIMD-batched leaf hashing.
- Extended `pvm-registry-regen` to mint and check the PVM setup commitment, circuit shape, stream-segment lengths, and common-segment digest used by the MASM verifier.
- Added `miden::core::sys::pvm::verify_proof`, a Poseidon2 MASM verifier for the ten-chiplet PVM relation, and `miden-precompiles-prover::masm_verifier::generate_masm_verifier_inputs`, its host-side proof adapter. The primitive consumes and authenticates a caller-supplied deferred root, returns transcript-bound proof parameters, and stops before proof-envelope and token wiring. The new `pvm::request_proof` event lets an async host supply the untrusted proof package on demand before verification.
- [BREAKING] Generalized the shared MASM STARK verifier layer over the relation's AIR count: `miden::core::sys::vm::constraints_eval_inputs` moved to `miden::core::stark::constraints_eval_inputs`; proof packages use the relation-agnostic `sys::build_proof_request_key`; `sys::vm::conjectured_security_level` moved to `stark::utils::conjectured_security_level`; relation evaluators now supply their AIR count, height array, stark-vars base, and quotient-recomposition inputs; `stark::constants::assert_valid_order_tag` takes its bound from the stack; and `stark::utils` gains N-generic proof-order ranking.
...
- [BREAKING] Added `miden::core::sys::pvm::verify_proof`, a MASM recursive verifier for the precompile VM, and generalized the shared MASM STARK verifier.
