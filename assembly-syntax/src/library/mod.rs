...
impl From<MastNodeId> for LibraryExport {
    fn from(node: MastNodeId) -> Self {
        Self { node, signature: None }
    }
}
...
impl Library {
    ...
    pub fn new(
        ...
        for (fqn, LibraryExport { node, .. }) in exports.iter() {
...
impl Library {
    ...
    pub fn exports(&self) -> impl Iterator<Item = (&QualifiedProcedureName, &LibraryExport)> {
        self.exports.iter()
    }
...
impl Library {
    ...
    pub fn module_infos(&self) -> impl Iterator<Item = ModuleInfo> {
        ...
        for (proc_name, LibraryExport { node, signature }) in self.exports.iter() {
            modules_by_path
...
impl Serializable for Library {
    fn write_into<W: ByteWriter>(&self, target: &mut W) {
        ...
        for (proc_name, LibraryExport { node, signature }) in exports {
...
impl Deserializable for Library {
    fn read_from<R: ByteReader>(source: &mut R) -> Result<Self, DeserializationError> {
        ...
        for _ in 0..num_exports {
            ...
            let export = LibraryExport { node, signature };
