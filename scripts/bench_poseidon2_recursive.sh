...
MONITOR_SYSTEM=0
RESULT_ROOT="$ROOT/bench-results/poseidon2-recursive"
ORIGINAL_ARGS="$*"

usage() {
  cat <<'EOF'
Usage:
  scripts/bench_poseidon2_recursive.sh [options]
...
mkdir -p "$RUN_DIR/logs" "$RUN_DIR/monitoring"
...
write_air_metadata() {
  local core_width chiplets_width core_aux chiplets_aux
...
run_once() {
  ...
  echo "[poseidon2-recursive] $fixture $label"
...
{
  echo "# Poseidon2 Recursive Verifier Benchmarks"
  echo
  echo "- Worktree: \`$ROOT\`"
...
echo
echo "[poseidon2-recursive] results: $RUN_DIR"
cat "$SUMMARY_MD"
