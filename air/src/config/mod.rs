...
//!
//! This module provides factory functions that create `LiftedConfig` instances
...
/// Bundled STARK configuration and Fiat-Shamir challenger.
///
/// Each hash function factory returns one of these. The caller uses `.prove()` or
/// `.verify()` directly instead of manually constructing transcript channels.
pub struct LiftedConfig<L, Ch> {
    pub config: StarkConfig<L, Dft>,
    pub challenger: Ch,
}

impl<L, Ch> LiftedConfig<L, Ch>
where
    L: Lmcs<F = Felt>,
    L::Commitment: Copy + Serialize,
    Ch: CanObserve<Felt>
        + CanObserve<L::Commitment>
        + GrindingChallenger<Witness = Felt>
        + CanSample<Felt>
        + CanSampleBits<usize>,
{
        ...
        B: AuxBuilder<Felt, QuadFelt>,
    {
        ...
        p3_miden_lifted_prover::prove_single::<_, QuadFelt, _, _, _, _, _>(
            &self.config,
            air,
            trace,
            public_values,
...
where
    L: Lmcs<F = Felt>,
    L::Commitment: Copy + DeserializeOwned,
    Ch: CanObserve<Felt>
        + CanObserve<L::Commitment>
        + GrindingChallenger<Witness = Felt>
        + CanSample<Felt>
        + CanSampleBits<usize>,
{
    ...
    /// Pre-seeds the challenger with `public_values`, then delegates to the lifted
    /// verifier.
    pub fn verify<A: LiftedAir<Felt, QuadFelt>>(
        self,
        air: &A,
        ...
        let transcript_data: p3_miden_transcript::TranscriptData<Felt, L::Commitment> =
        ...
        p3_miden_lifted_verifier::verify_single::<_, QuadFelt, _, _, _, _>(
            &self.config,
            air,
            log_trace_height,
            public_values,
...
mod tests {
    ...
    fn test_poseidon2_prove_verify() {
        ...
        let proof = config.prove(&air, &trace, &pv, &aux).expect("prove");

        let config = super::create_poseidon2_config();
        let log_h = trace.height().trailing_zeros() as usize;
        config.verify(&air, log_h, &pv, &proof).expect("verify");
    }
    ...
    fn test_blake3_lmcs_keccak_challenger() {
        ...
        let config = StarkConfig { pcs: super::PCS_PARAMS, lmcs, dft };
        let challenger = Ch::from_hasher(vec![], Keccak256Hash {});
        let lifted = super::LiftedConfig { config, challenger };
        ...
        let proof = lifted.prove(&air, &trace, &pv, &aux).expect("prove");

        // Rebuild config for verify
        let sponge = Sponge::new(Blake3);
        let compress = Compress::new(Blake3);
        let lmcs = LmcsType::new(sponge, compress);
        let dft = super::Dft::default();
        ...
        let log_h = trace.height().trailing_zeros() as usize;
        lifted.verify(&air, log_h, &pv, &proof).expect("verify");
    }
}
