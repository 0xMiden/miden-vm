...
//! columns check `U * aux - V = 0`; column 0 is the running accumulator. Last-row-idle AIRs bind
//! the raw final sum on the last row. Wrapped-centered AIRs subtract the committed center on every
//! cyclic edge.
//!
//! Message encoding stays in [`LookupMessage::encode`]. The adapter only supplies cached
//! challenges and folds the resulting denominators into each column's `(V, U)` pair.
...
use super::{
    Challenges, Deg, LookupAccumulatorMode, LookupAir, LookupBatch, LookupBuilder, LookupColumn,
    ...
    AB: LiftedAirBuilder + 'ab,
{
    ...
    accumulator_mode: LookupAccumulatorMode,
    column_idx: usize,
}
    ...
    AB: LiftedAirBuilder,
{
        ...
        A: LookupAir<Self>,
    {
        ...
        let accumulator_mode = air.accumulator_mode();
    ...
    AB: LiftedAirBuilder,
{
    ...
    fn next_column<'a, R>(
        ...
        if col_idx == 0 {
            ...
            match self.accumulator_mode {
                LookupAccumulatorMode::LastRowIdle => {
                    // Last-row-idle mode commits the raw sum. The final row is padding, so the
                    // update is enforced only on transition rows and the final accumulator is
                    // bound directly to the committed value.
                ...
                LookupAccumulatorMode::WrappedCentered => {
                    // Wrapped mode commits the center value S / N and subtracts it on every row.
                    // This makes the cyclic last-to-first transition close without an idle row.
                    self.ab.when_first_row().assert_zero_ext(acc);
                    self.ab.assert_zero_ext(u * (acc_next - all_curr_sum + committed_final) - v);
                },
            }
        } else {
            ...
            match self.accumulator_mode {
                LookupAccumulatorMode::LastRowIdle => {
                    // Preserve the historical last-row-idle constraint shape for existing AIRs.
