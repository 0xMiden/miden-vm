...
//! - Deferred-root update: gated by the log precompile opcode.
...
//!   - Deferred-root batch: 1 remove + 1 add gated by `log_deferred`.
...
//! | deferred-root batch (k=2, f=log_deferred deg 5) | — | DeferredRoot, denom 1 | **7** | **6** |
//!
...
use crate::{
    constraints::lookup::{
        ...
        messages::{BlockStackMsg, DeferredRootMsg, RangeMsg},
    },
    lookup::{Deg, LookupBatch, LookupColumn, LookupGroup},
    trace::log_deferred::{HELPER_DEFERRED_ROOT_PREV_RANGE, STACK_DEFERRED_ROOT_NEW_RANGE},
};
    ...
    LB: MainLookupBuilder,
{
    ...
    // LOGDEFERRED deferred-root add/remove payloads.
    let root_prev: [LB::Var; 4] =
        array::from_fn(|i| user_helpers[HELPER_DEFERRED_ROOT_PREV_RANGE.start + i]);
    let root_new: [LB::Var; 4] =
        array::from_fn(|i| stk_next.get(STACK_DEFERRED_ROOT_NEW_RANGE.start + i));
        ...
        |col| {
            ...
            col.group(
                ...
                |g| {
                    ...
                    // ---- Deferred-root update (BusId::DeferredRoot) ----
                    ...
                    g.batch(
                        ...
                        f_log_deferred,
                        move |b| {
                            let root_prev_expr = root_prev.map(LB::Expr::from);
                            b.remove(
                                "deferred_root_remove",
                                DeferredRootMsg { root: root_prev_expr },
                                Deg { v: 5, u: 6 },
                            );
                            let root_new_expr = root_new.map(LB::Expr::from);
                            b.add(
                                "deferred_root_add",
                                DeferredRootMsg { root: root_new_expr },
                                Deg { v: 5, u: 6 },
                            );
                        },
