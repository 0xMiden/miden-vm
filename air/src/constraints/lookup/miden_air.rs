...
pub(crate) fn emit_core_boundary<B: BoundaryBuilder>(boundary: &mut B) {
    // The core boundary's statement inputs are `[program_hash (4) | transcript_state (4)]`.
    // Absent inputs (debug walker on a bare trace) default to zero, mirroring
    // `emit_chiplets_boundary`'s handling of an empty digest group.
