...
//! - block-stack table + u32 range-check removes + log-precompile capacity (merged — see
//!   [`super::buses::block_stack_and_range_logcap`]).
...
/// checks + log-precompile capacity; the second unions block-hash queue and op-group table; the
/// third hosts the decoder's chiplet requests; the fourth hosts the stack overflow table. The
/// chiplet-trace half of the argument lives in
    ...
    LB: MainLookupBuilder,
{
    ...
    fn max_message_width(&self) -> usize {
        // The widest main-trace payload is `HasherMsg::State` at 15 slots; keep the
        // rounded-up width for MASM transcript alignment.
