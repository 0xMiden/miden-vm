...
//!   walk.
//! - [`symbolic`] — [`symbolic::check_symbolic_degrees`] degree-budget pass over the production
//!   `ConstraintLookupBuilder`.
//! - This file — records ([`DebugStructure`], [`GroupRecord`], …), existing check wrappers
//!   ([`inspect_structure`], [`collect_inventory`], [`check_encoding_equivalence`],
//!   [`check_challenge_scoping`]), and [`validate`] / [`ValidationReport`].
...
use super::super::{ConstraintLookupBuilder, Deg, LookupAir, LookupChallenges};
use crate::Felt;
    ...
    for<'ab> A: LookupAir<ConstraintLookupBuilder<'ab, SymbolicAirBuilder<Felt, QuadFelt>>>,
{
    ...
    let declared_columns = num_columns_via::<A>(air);
...
/// Helper that pins the single-bound `LookupAir<DebugStructureBuilder<'_>>` dispatch so
/// `num_columns()` resolves unambiguously inside [`validate`] (where `A` satisfies both
/// the structure and symbolic bounds simultaneously).
fn num_columns_via<A>(air: &A) -> usize
where
    for<'a> A: LookupAir<DebugStructureBuilder<'a>>,
{
    air.num_columns()
