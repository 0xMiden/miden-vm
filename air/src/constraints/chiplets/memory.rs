...
//! Range-bus lookups should include `w0`, `w1`, and `4 * w1`; the extra term
    ...
    AB: MidenAirBuilder<F = Felt>,
{
    ...
    // Binary constraints
    builder.when(memory_flag.clone()).assert_zeros([
        cols.is_read.clone() * (cols.is_read.clone() - one.clone()),
        cols.is_word.clone() * (cols.is_word.clone() - one.clone()),
        cols.idx0.clone() * (cols.idx0.clone() - one.clone()),
        cols.idx1.clone() * (cols.idx1.clone() - one),
    ]);
    ...
    builder.when(word_gate).assert_zeros([cols.idx0.clone(), cols.idx1.clone()]);
}
    ...
    AB: MidenAirBuilder<F = Felt>,
{
    ...
    builder.when(flag_next_row_first_memory).assert_zeros([
        c0 * cols_next.values[0].clone(),
        c1 * cols_next.values[1].clone(),
        c2 * cols_next.values[2].clone(),
        c3 * cols_next.values[3].clone(),
    ]);
}
    ...
    AB: MidenAirBuilder<F = Felt>,
{
    ...
    builder.when(flag_memory_active_not_last.clone()).assert_zeros([
        // n0 is binary
        n0.clone() * (n0.clone() - one.clone()),
        // When context changes, n0 must be 1 (equivalently: when n0=0, ctx_delta=0)
        (one.clone() - n0.clone()) * ctx_delta.clone(),
        // When n0=0, n1 is binary
        (one.clone() - n0.clone()) * n1.clone() * (n1.clone() - one.clone()),
        // When n0=0 and n1=0, addr_delta=0
        (one.clone() - n0.clone()) * (one.clone() - n1.clone()) * addr_delta.clone(),
    ]);
    ...
    let computed_delta = n0.clone() * ctx_delta
        + (one.clone() - n0.clone())
            * (n1.clone() * addr_delta + (one.clone() - n1.clone()) * clk_delta.clone());
    builder.assert_zero(flag_memory_active_not_last.clone() * (computed_delta - delta_next));
    ...
    builder.assert_zero(
        flag_memory_active_not_last.clone()
            * (cols_next.flag_same_ctx_word.clone()
                - (one.clone() - n0.clone()) * (one.clone() - n1.clone())),
    );

    // ==========================================================================
    // SAME CONTEXT/WORD/CLOCK ACCESS CONSTRAINTS
    // ==========================================================================

    // When f_scw' = 1 and clk_delta = 0, both ops must be reads.
    // Constraint: f_scw' * (1 - clk_delta * d_inv') * (1 - rw) * (1 - rw') = 0
    // TODO(Al): consider forbidding any same-cycle ops (reads included) for soundness.
    let f_scw_next = cols_next.flag_same_ctx_word.clone();
    let clk_no_change = one.clone() - clk_delta.clone() * cols_next.d_inv.clone();
    let both_writes =
        (one.clone() - cols.is_read.clone()) * (one.clone() - cols_next.is_read.clone());
    builder.assert_zero(
        flag_memory_active_not_last.clone() * f_scw_next.clone() * clk_no_change * both_writes,
    );
    ...
    let constrain_value = |c: AB::Expr, v: AB::Expr, v_next: AB::Expr| {
        flag_memory_active_not_last.clone()
            * c
            * (f_scw_next.clone() * (v_next.clone() - v)
                + (one.clone() - f_scw_next.clone()) * v_next)
    };
...
impl<E: Clone> MemoryColumns<E> {
    ...
    {
        MemoryColumns {
            is_read: load_col::<AB, E>(row, MEMORY_IS_READ_COL_IDX),
            is_word: load_col::<AB, E>(row, MEMORY_IS_WORD_ACCESS_COL_IDX),
            ctx: load_col::<AB, E>(row, MEMORY_CTX_COL_IDX),
            word_addr: load_col::<AB, E>(row, MEMORY_WORD_COL_IDX),
            idx0: load_col::<AB, E>(row, MEMORY_IDX0_COL_IDX),
            idx1: load_col::<AB, E>(row, MEMORY_IDX1_COL_IDX),
            clk: load_col::<AB, E>(row, MEMORY_CLK_COL_IDX),
            values: core::array::from_fn(|i| load_col::<AB, E>(row, MEMORY_V_COL_RANGE.start + i)),
            d0: load_col::<AB, E>(row, MEMORY_D0_COL_IDX),
            d1: load_col::<AB, E>(row, MEMORY_D1_COL_IDX),
            d_inv: load_col::<AB, E>(row, MEMORY_D_INV_COL_IDX),
            flag_same_ctx_word: load_col::<AB, E>(row, MEMORY_FLAG_SAME_CONTEXT_AND_WORD),
        }
    }
}

/// Helper to load a single column from a row.
fn load_col<AB, E>(row: &MainTraceRow<AB::Var>, global_idx: usize) -> E
where
    AB: MidenAirBuilder<F = Felt>,
    AB::Var: Into<E> + Clone,
{
