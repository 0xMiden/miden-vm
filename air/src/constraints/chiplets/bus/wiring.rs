...
//! `coeffs[0] + coeffs[1] * clk + coeffs[2] * ctx + coeffs[3] * id + coeffs[4] * v0 + coeffs[5] *
//! v1`
//!
//! Where:
...
use crate::{
    Felt, MainTraceRow,
    constraints::{
        bus::{coeffs_from_challenges, encode_message, indices::V_WIRING},
...
/// Number of message coefficients for the wiring bus.
/// Format: coeffs[0] + coeffs[1]*clk + coeffs[2]*ctx + coeffs[3]*id + coeffs[4]*v0 + coeffs[5]*v1
const NUM_WIRING_COEFFS: usize = 6;
    ...
    AB: TaggingAirBuilderExt<F = Felt>,
{
    ...
    let (v_local, v_next, coeffs) = {
        ...
        let coeffs: [AB::ExprEF; NUM_WIRING_COEFFS] =
            coeffs_from_challenges::<AB, NUM_WIRING_COEFFS>(challenges);
        (v_local, v_next, coeffs)
    };
    ...
    let wire_0: AB::ExprEF = encode_wire::<AB>(&coeffs, &clk, &ctx, &wire_0);
    let wire_1: AB::ExprEF = encode_wire::<AB>(&coeffs, &clk, &ctx, &wire_1);
    let wire_2: AB::ExprEF = encode_wire::<AB>(&coeffs, &clk, &ctx, &wire_2);
