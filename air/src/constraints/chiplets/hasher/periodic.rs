...
/// Length of one hash cycle (32 rows for Poseidon2).
pub const HASH_CYCLE_LEN: usize = 32;

/// Width of the Poseidon2 state (12 field elements).
pub const STATE_WIDTH: usize = 12;

// Periodic column indices (keep in sync with `periodic_columns()` output order).
...
///   - Row 0 is the initial "linear" step (M_E only) and row 31 is idle; both selectors are 0.
///
