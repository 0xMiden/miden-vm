...
/// ## Degree Analysis
///
/// The highest degree comes from the S-box (degree 7) combined with gating:
/// - step_gate: ~2
/// - is_external/is_internal: ~1 (periodic)
/// - S-box: 7
/// - Total per constraint: ~10
...
{
    // Cycle markers and step selectors (base field)
    ...
    // External round constants (from periodic columns)
    ...
    // -------------------------------------------------------------------------
    // Enforce step constraints (gated by step type)
    // -------------------------------------------------------------------------
