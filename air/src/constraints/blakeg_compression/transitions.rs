...
/// - `B.a = a + b + msg - 2^32 * k3`
/// - `pack(B.b_bytes) = A.b`
/// - `B.d = rot16(pack(A.d_bytes) ^ pack(A.a_new_bytes))`
/// - `A.c + B.d = pack(B.c_new_bytes) + 2^32 * B.k2`
    ...
    AB: LiftedAirBuilder<F = Felt>,
{
    ...
    for g in 0..NUM_G {
        // B.a = A add3 result.
        gated_eq(builder, gate.clone(), a_local.add3_result_word(g), b_next.a(g));

        // pack(B.b_bytes) = A.b.
        gated_eq(builder, gate.clone(), a_local.b(g), b_next.packed_b_bytes(g));

        // B.d = rot16(pack(A.d_bytes) ^ pack(A.a_new_bytes)).
        let d_new = a_local.rot16_d_xor_a_new_word(g);
        ...
        gated_eq(
            ...
            gate.clone(),
            a_local.c(g) + d_new,
            b_next.packed_c_new_bytes(g) + two_pow_32::<AB>() * b_next.k2(g),
        );
    }
}
...
///
/// - `C.a = B.a`
/// - `C.b` is the sum of the four B-row rotation contributions
/// - `C.c = pack(B.c_new_bytes)`
/// - `pack(C.d_bytes) = B.d`
    ...
    AB: LiftedAirBuilder<F = Felt>,
{
    ...
    for g in 0..NUM_G {
        ...
        // C.b = rot12(pack(B.b_bytes) ^ pack(B.c_new_bytes)).
        gated_eq(builder, gate.clone(), b_local.rotated_b_xor_c_new_word(g), c_next.b(g));

        // C.c = pack(B.c_new_bytes).
        gated_eq(builder, gate.clone(), b_local.packed_c_new_bytes(g), c_next.c(g));

        // pack(C.d_bytes) = B.d.
        gated_eq(builder, gate.clone(), b_local.d(g), c_next.packed_d_bytes(g));
    }
}
    ...
    AB: LiftedAirBuilder<F = Felt>,
{
    ...
    for g in 0..NUM_G {
        // D.a = C add3 result.
        gated_eq(builder, gate.clone(), c_local.add3_result_word(g), d_next.a(g));

        // pack(D.b_bytes) = C.b.
        gated_eq(builder, gate.clone(), c_local.b(g), d_next.packed_b_bytes(g));

        // D.d = rot8(pack(C.d_bytes) ^ pack(C.a_new_bytes)).
        let d_new = c_local.rot8_d_xor_a_new_word(g);
        gated_eq(builder, gate.clone(), d_new.clone(), d_next.d(g));

        // C.c + D.d = pack(D.c_new_bytes) + 2^32 * D.k2.
        gated_eq(
            builder,
            gate.clone(),
            c_local.c(g) + d_new,
            d_next.packed_c_new_bytes(g) + two_pow_32::<AB>() * d_next.k2(g),
        );
    }
}
