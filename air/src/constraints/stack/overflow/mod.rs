...
//!    - Left shift with overflow: depth decreases by 1
...
/// - Left shift operations with overflow: depth -= 1
    ...
    AB: MidenAirBuilder,
{
    ...
    // For normal operations (not CALL/SYSCALL/DYNCALL and not END-of-call),
    // the depth update follows the shift flags:
    ...
    // Left shift with overflow: when f_shl=1 and f_ov=1, depth must decrement by 1.
    ...
    builder
        .when_transition()
        .assert_zero(no_shift_part + left_shift_part - right_shift_part + call_part);
}
