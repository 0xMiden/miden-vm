...
//! `coeffs[0] + coeffs[1] * clk + coeffs[2] * val + coeffs[3] * prev`
...
use crate::{
    MainTraceRow,
    constraints::{
        bus::{coeffs_from_challenges, encode_message, indices::P1_STACK},
        op_flags::OpFlags,
    ...
    AB: MidenAirBuilder,
{
    ...
    let (p1_local, p1_next, coeffs) = {
        ...
        // We need 3 coefficients for row encoding: (clk, val, prev), plus one extra coefficient.
        let coeffs: [AB::ExprEF; 4] = coeffs_from_challenges::<AB, 4>(challenges);
        (p1_local, p1_next, coeffs)
    };
    ...
    // -------------------------------------------------------------------------
    // Row value encoding: coeffs[0] + coeffs[1] * clk + coeffs[2] * val + coeffs[3] * prev
    // -------------------------------------------------------------------------

    // Response row value (adding to table during right_shift):
    let response_row = encode_message::<AB, 3>(&coeffs, [clk.clone(), s15.clone(), b1.clone()]);

    // Request row value for left_shift (removing from table):
    let request_row_left =
        encode_message::<AB, 3>(&coeffs, [b1.clone(), s15_next.clone(), b1_next.clone()]);

    // Request row value for dyncall (removing from table):
    let request_row_dyncall =
        encode_message::<AB, 3>(&coeffs, [b1.clone(), s15_next.clone(), hasher_state_5.clone()]);
