    ...
    AB: LiftedAirBuilder,
{
    ...
    let range_check: AB::ExprEF = alpha + local.range[RANGE_V_COL_IDX].into();
    ...
    let not_4: AB::Expr = AB::Expr::ONE - local.decoder[OP_BIT_4_COL_IDX].into();
    let not_5: AB::Expr = AB::Expr::ONE - local.decoder[OP_BIT_5_COL_IDX].into();
