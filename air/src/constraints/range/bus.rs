...
use crate::{
    MainTraceRow,
    trace::{CHIPLETS_OFFSET, RANGE_CHECK_TRACE_OFFSET, chiplets, decoder, range},
};
...
const CHIPLET_S0_IDX: usize = 0;
const CHIPLET_S1_IDX: usize = 1;
const CHIPLET_S2_IDX: usize = 2;
    ...
    AB: MidenAirBuilder,
{
    ...
    let (b_local, b_next, alpha) = {
        ...
        let b_local: AB::ExprEF = aux_local[range::B_RANGE_COL_IDX].into();
        let b_next: AB::ExprEF = aux_next[range::B_RANGE_COL_IDX].into();
    ...
    let sflag_rc_mem: AB::ExprEF = range_check.clone() * memory_lookups.clone() * u32_rc_op;
    ...
    let mflag_rc_stack: AB::ExprEF = range_check * stack_lookups.clone() * chiplets_memory_flag;

    // LogUp transition constraint terms
    let b_next_term: AB::ExprEF = b_next * lookups.clone();
    let b_term: AB::ExprEF = b_local * lookups;
    let rc_term: AB::ExprEF =
        stack_lookups * memory_lookups * local.range[RANGE_M_COL_IDX].clone().into();

    // Stack lookup removal terms
    let s0_term: AB::ExprEF = sflag_rc_mem.clone() * sv1.clone() * sv2.clone() * sv3.clone();
    let s1_term: AB::ExprEF = sflag_rc_mem.clone() * sv0.clone() * sv2.clone() * sv3.clone();
    let s2_term: AB::ExprEF = sflag_rc_mem.clone() * sv0.clone() * sv1.clone() * sv3;
    let s3_term: AB::ExprEF = sflag_rc_mem * sv0 * sv1 * sv2;

    // Memory lookup removal terms
    let m0_term: AB::ExprEF = mflag_rc_stack.clone() * mv1;
    let m1_term: AB::ExprEF = mflag_rc_stack * mv0;
