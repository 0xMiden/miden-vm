...
/// scalars in Rust. Rehydrating successful deferred work also checks the native precompile
/// relation.
#[test]
    ...
    for group in vectors.test_groups {
        for test in group.tests {
            ...
            if let Ok(output) = result {
                ...
                let wire = output.deferred_state.to_wire().unwrap();
                let restored = DeferredState::from_wire(Arc::clone(&registry), &wire)
                    .unwrap_or_else(|err| panic!("tcId {id}: invalid deferred relation: {err}"));
                assert_eq!(
                    restored.root(),
                    output.deferred_state.root(),
                    "tcId {id}: root changed"
                );
            }
        }
    }
