...
After release, version bumps are additive only: a newer ABI revision may add host functions but must not change or remove existing ones. Hosts accept supported declarations from `1` up to their own version and check that every imported function is available in the declared revision. Revision 2 adds `invocation_kind`; declaring revision 1 while importing it is rejected. Package derivation selects the minimum revision required by a module's imports. A breaking change to a released ABI gets a new import namespace (`miden:event/v2`) instead.
