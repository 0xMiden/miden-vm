...
| `log_precompile`   | `[COMMITMENT, TAG, ...]` | `[CAP, R0, R1, ...]` | 8      | Absorbs a precompile commitment into the sequential commitment sponge for deferred verification. Takes a precompile commitment (TAG and COMMITMENT, 8 elements total) from the stack and performs an RPO256 permutation to update the internal sponge state. Outputs the updated capacity CAP (4 elements) and rate outputs R0, R1 (8 elements) to the stack. Used internally by precompile event handlers to track operations that will be verified during proof verification. |
