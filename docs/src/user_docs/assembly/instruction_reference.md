...
| `log_precompile`   | `[_, STMNT, _, ...]` | `[STATE_NEW, OUT_RATE1, OUT_CAP, ...]` | 1      | Folds `STMNT` from `stack[4..8]` into the VM's rolling precompile transcript using the internally maintained previous state and capacity `[1, 0, 0, 0]`. Writes three output words, normally dropped by wrappers. Prefer `sys::log_precompile_request` unless implementing a precompile wrapper. |
