...
| Instruction | Stack Input | Stack Output | Cycles | Notes                                                                                                                                    |
| ----------- | ----------- | ------------ | ------ | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `emit.<event_id>` | `[...]`     | `[...]`      | 5      | Emits an event with the specified `event_id` to the host. Does not change the state of the operand stack. Supports both legacy 32-bit values and enhanced EventId strings with hierarchical naming and collision detection. The `event_id` can be any 32-bit value specified either directly or via a [named constant](./code_organization.md#constants). Events allow programs to communicate contextual information to the host for triggering appropriate actions. Example: `emit.123`, `emit.EVENT_ID_1`, or `emit."user-0/trading::ORDER_EXECUTED"`. See [Events documentation](./events.md) for details on the enhanced event system. |
