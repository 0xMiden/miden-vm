...
| `log_precompile`   | `[COMM_CALLDATA, TAG, PAD, ...]` | `[R1, R0, CAP_NEXT, ...]` | 1      | Absorbs a precompile commitment into the sequential commitment sponge for deferred verification. Takes a precompile commitment (`TAG` and `COMM_CALLDATA`) from the stack and performs an RPO256 permutation to update the precompile sponge capacity. Outputs `[R1, R0, CAP_NEXT]`; callers normally drop all three words immediately. See “Precompile flow” for initialization details. |
