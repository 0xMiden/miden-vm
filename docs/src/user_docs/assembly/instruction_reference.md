...
| `log_deferred`   | `[_, STATEMENT, _, ...]` | `[DEFERRED_ROOT_NEW, OUT_RATE1, OUT_CAP, ...]` | 1      | Folds `STATEMENT` from `stack[4..8]` into the VM's rolling deferred root via `DEFERRED_ROOT_NEW = Node::and(DEFERRED_ROOT_PREV, STATEMENT).digest()`, implemented as `rate0(Poseidon2([DEFERRED_ROOT_PREV, STATEMENT, [1,0,0,0]]))` with framework `Tag::AND` capacity. Writes three output words, normally dropped by wrappers. |
