...
| `aead_stream` | `[K_CTR(4), counter, src_ptr, dst_ptr, remaining, ...]` | `[K_CTR(4), counter+1, src_ptr+8, dst_ptr+16, remaining-1, ...]` | 1 | AEAD stream step: calls the AEAD stream chip to derive BlakeG-XOF blocks from `K_CTR` and `counter`, reads 8 packed plaintext felts from `src_ptr`, writes 16 ciphertext-limb felts to `dst_ptr`, and rejects overlapping source/destination ranges. Primitive used by `miden::core::crypto::aead_blakeg`. |
...
| `log_precompile`   | `[_, STMNT, ...]` | `[STATE_NEW, STMNT, ...]` | 1      | Folds a precomputed precompile statement into the rolling transcript. Reads `STMNT` from `stack[4..8]`, applies BlakeG compression to block `[STATE_PREV, STMNT]` with the constant Eidos two-to-one chaining word, and writes `STATE_NEW` to the top word. See "Precompile flow" for initialization details. |
