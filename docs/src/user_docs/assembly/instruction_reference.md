...
| Instruction        | Stack Input       | Stack Output | Cycles | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
|--------------------|-------------------|--------------|--------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `emit.<EVENT_CONST>` | `[...]`           | `[...]`      | 5      | Emits an event using an event constant defined with `event()` syntax. The net effect on the operand stack is no change (internally, the event_id is pushed then immediately popped). Example: `emit.MY_ERROR_EVENT` where `MY_ERROR_EVENT` is defined as `const.MY_ERROR_EVENT=event(100)` |
| `emit.event(<id>)` | `[...]`           | `[...]`      | 5      | Emits an event with the specified immediate `event_id` to the host. The net effect on the operand stack is no change (internally, the event_id is pushed then immediately popped). The `event_id` can be any 32-bit value. Example: `emit.event(123)` |
| `emit`             | `[event_id, ...]` | `[...]`      | 2      | Emits an event using the `event_id` from the top of the stack. The event_id is consumed from the stack. This is equivalent to the immediate version but takes the event ID from the stack instead of as an immediate value. Example: `push.123 emit`                                                                                                                                                                                                        |
