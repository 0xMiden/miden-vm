...
| `log_precompile`   | `[_, STMNT, _, ...]` | `[STATE_NEW, OUT_RATE1, OUT_CAP, ...]` | 1      | Folds a statement digest into the rolling deferred commitment. Reads the per-call statement word `STMNT` from `stack[4..8]` (the HPERM rate1 slots), applies the framework-AND Poseidon2 permutation over `[STATE_PREV, STMNT, Tag::AND]` (with `STATE_PREV` supplied non-deterministically via helper registers), and writes the permutation output back via the identity lane→slot mapping (matching HPERM): `STATE_NEW` (= output `rate0`) at `stack[0..4]`, then `rate1` and `capacity` halves at `stack[4..8]` and `stack[8..12]`. Callers normally drop all three words immediately. See [Deferred computation](../../design/deferred/index.md) for initialization details. |
