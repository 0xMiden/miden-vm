...
| adv.register_deferred                        | [PAYLOAD_LO, PAYLOAD_HI, TAG, ...] | [PAYLOAD_LO, PAYLOAD_HI, TAG, ...] | Registers an expression node in the deferred DAG and pushes `NODE_DIGEST` to the advice stack. |
| adv.register_deferred_chunk                  | [TAG, ptr, ...]    | [TAG, ptr, ...]    | Registers a chunk node in the deferred DAG (chunk length decoded from `TAG`, chunk data read from memory at `ptr`) and pushes `NODE_DIGEST` to advice. |
| adv.evaluate_deferred                        | [NODE_DIGEST, ...] | [NODE_DIGEST, ...] | Evaluates the node, pushes `CANONICAL_DIGEST` to advice, and records `advice_map[CANONICAL_DIGEST] = tag \|\| payload`. |
