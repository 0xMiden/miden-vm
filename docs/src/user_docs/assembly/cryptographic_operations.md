...
| log_precompile <br> - *(1 cycle)*   | [COMM_CALLDATA, TAG, ...]                                                                            | [R1, R0, CAP_NEXT, ...]                                                                               | Absorbs words `TAG` and `COMM_CALLDATA` into the precompile sponge.<br/>The hasher computes `[CAP_NEXT, R0, R1] = Rpo([CAP_PREV, TAG, COMM_CALLDATA])` and updates the processor's precompile sponge capacity.<br/>The top 3 stack words are replaced with `[R1, R0, CAP_NEXT]`, and callers typically drop them right away.                                             |
