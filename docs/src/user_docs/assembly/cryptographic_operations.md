...
| log_precompile <br /> - *(1 cycle)*   | [_, STMNT, _, ...]                                                                                 | [STATE_NEW, OUT_RATE1, OUT_CAP, ...]                                                                               | Folds the statement digest `STMNT` (read from `stack[4..8]`) into the rolling deferred commitment using the internally maintained previous state and framework AND capacity `[1, 0, 0, 0]`.<br />The top 3 stack words are replaced with `[STATE_NEW, OUT_RATE1, OUT_CAP]`; `OUT_RATE1` and `OUT_CAP` are unused and typically dropped. See [Deferred computation](../../design/deferred/index.md) for the model.                   |
