...
| log_deferred <br /> - *(1 cycle)*   | [_, STATEMENT, _, ...]                                                                              | [DEFERRED_ROOT_NEW, OUT_RATE1, OUT_CAP, ...]                                                                        | Folds `STATEMENT` from stack offsets 4..8 into the VM's rolling deferred root via `DEFERRED_ROOT_NEW = Node::and(DEFERRED_ROOT_PREV, STATEMENT).digest()`, implemented as `rate0(Poseidon2([DEFERRED_ROOT_PREV, STATEMENT, [1,0,0,0]]))` with framework `Tag::AND` capacity.<br />The top 3 stack words are replaced with `[DEFERRED_ROOT_NEW, OUT_RATE1, OUT_CAP]`, and wrappers typically drop them right away.                   |
