...
| log_precompile <br /> - *(1 cycle)*   | [_, STMNT, _, ...]                                                                                 | [STATE_NEW, OUT_RATE1, OUT_CAP, ...]                                                                               | Folds `STMNT` from stack offsets 4..8 into the VM's rolling precompile transcript via `STATE_NEW = rate0(Poseidon2([STATE_PREV, STMNT, [1,0,0,0]]))`, using the internally maintained previous state and fixed capacity word `[1, 0, 0, 0]`.<br />The top 3 stack words are replaced with `[STATE_NEW, OUT_RATE1, OUT_CAP]`, and wrappers typically drop them right away. Prefer `sys::log_precompile_request` unless implementing a precompile wrapper.                   |
