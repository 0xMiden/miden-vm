...
| log_precompile <br> - *(1 cycle)*   | [HASH_CALL_DATA, TAG, ...]                                                                          | [R1, R0, CAP_NEXT, ...]                                                                               | Absorbs a 4-element `TAG` and 4-element `HASH_CALL_DATA` into the running precompile accumulator.<br/>The hasher computes `[CAP_NEXT, R0, R1] = Rpo([CAP_PREV, TAG, HASH_CALL_DATA])` and updates the processor's capacity word.<br/>Stack words are exposed in reverse element order, so the top four stack elements correspond to `R1[3..0]`, followed by `R0[3..0]`, then `CAP_NEXT[3..0]`. |
