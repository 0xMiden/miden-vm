...
| log_precompile_request | Logs a precompile commitment and removes the helper words produced by the underlying `log_precompile` instruction. The opcode updates the transcript state by applying the domain-separated Poseidon2 merge `STATE_NEW = rate0(Poseidon2([STATE_PREV, STMNT, [1,0,0,0]]))` using the fixed capacity word `[1, 0, 0, 0]`.<br/><br/>Input: `[COMM, TAG, ...]`<br/>Output: `[...]` (top three helper words are dropped internally). |
