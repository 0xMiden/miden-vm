...
| log_node_digest | Folds a deferred node digest into the rolling deferred commitment using the existing `log_precompile` commitment state. The node must evaluate to `TRUE` under the installed precompiles.<br/><br/>Input: `[NODE_DIGEST, ...]`<br/>Output: `[...]`. |
