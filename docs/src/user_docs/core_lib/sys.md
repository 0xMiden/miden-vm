...
| log_node_digest | Folds a deferred node digest into the rolling deferred commitment using the existing `log_precompile` transcript state. The node must reduce to `TRUE` under the installed schema.<br/><br/>Input: `[NODE_DIGEST, ...]`<br/>Output: `[...]`. |
