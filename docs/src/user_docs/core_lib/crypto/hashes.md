...
| absorb_double_words_from_memory | Hashes the aligned memory range `[start_addr, end_addr)` from an existing Eidos state.<br /><br />This requires `end_addr = start_addr + 8n` for `n = 0, 1, 2, ...`; otherwise the procedure will not terminate.<br /><br />**Inputs:** `[R0, R1, C, start_addr, end_addr, ...]`<br />**Outputs:** `[R0', R1', C', start_addr', end_addr, ...]`, where `start_addr' = end_addr`.<br /><br />Where:<ul><li>`R0` is the first block word (positions 0-3, on top of stack).</li><li>`R1` is the second block word (positions 4-7).</li><li>`C` is the chaining word (positions 8-11).</li></ul><br />Cycles: 4 + 3 * words, where `words = end_addr - start_addr`    |
| hash_double_words               | Hashes an aligned memory range using the same Eidos framing as `hash_words`.<br /><br />This helper is kept for callers whose data is naturally stored as 8-Felt records, but the committed length is still the checked range `[start_addr, end_addr)`.<br /><br />**Inputs:** `[start_addr, end_addr, ...]`<br />**Outputs:** `[HASH, ...]` |
| hash_words                      | Hashes the word-aligned memory range `[start_addr, end_addr)`.<br /><br />Requires `start_addr <= end_addr`; `end_addr` is not inclusive.<br /><br />**Inputs:** `[start_addr, end_addr, ...]`<br />**Outputs:** `[H, ...]`<br /><br />Cycles:<ul><li>even words: 55 cycles + 3 * words</li><li>odd words: 67 cycles + 3 * words</li></ul>       |
| hash_words_with_domain          | Hashes the word-aligned memory range `[start_addr, end_addr)` with a domain identifier.<br /><br />**Inputs:** `[domain, start_addr, end_addr, ...]`<br />**Outputs:** `[H, ...]` |
| init_chaining_word              | Constructs `Eidos::init_chaining_word(0, n)`.<br /><br />**Inputs:** `[n, ...]`<br />**Outputs:** `[CV, ...]` |
| init_chaining_word_in_domain    | Constructs `Eidos::init_chaining_word(domain, n)`.<br /><br />**Inputs:** `[n, domain, ...]`<br />**Outputs:** `[CV, ...]` |
...
| hash_elements_with_domain       | Computes hash of `Felt` values starting at the specified memory address with a domain identifier.<br /><br />**Inputs:** `[ptr, num_elements, domain]`<br />**Outputs:** `[HASH]` |
...
| merge_in_domain                 | Merges two words via Eidos hash with a domain identifier.<br /><br />**Inputs:** `[domain, A, B, ...]`<br />**Outputs:** `[C, ...]` |
