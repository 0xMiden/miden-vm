...
| verify_digest | Proves the existence of a secp256k1 ECDSA-valid `(r, s)` witness for a precomputed Keccak256 digest. The digest is supplied on the operand stack, while the public key and signature scalars are supplied via advice.<br /><br />**Stack inputs:** `[PK_COMM, DIGEST_U32[8], ...]`<br />**Advice stack inputs:** `[QX[8], QY[8], SIG_R[8], SIG_S[8], ...]`<br />**Outputs:** `[...]`<br /><br />`DIGEST_U32[8]` is the packed little-endian u32 output returned by `crypto::hashes::keccak256::hash_bytes`. The procedure converts those limbs to the ECDSA scalar representation without hashing them again. The same public-key commitment, scalar validation, and low-s behavior as `verify` apply. |
