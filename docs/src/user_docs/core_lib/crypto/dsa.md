...
* `Falcon512`: a variant of the [Falcon](https://falcon-sign.info/) signature scheme with native VM hashing.
...
Module `miden::core::crypto::dsa::falcon512_eidos` contains procedures for verifying Miden's native-hash Falcon512 signatures. These signatures differ from standard Falcon signatures in that the *hash-to-point* algorithm uses the VM-native hash instead of `SHAKE256`.

The module exposes the following procedures:

| Procedure   | Description |
| ----------- | ------------- |
| verify      | Verifies a signature against a public key and a message. The procedure gets as inputs the hash of the public key and the hash of the message via the operand stack. The signature is expected to be provided via the advice provider.<br /><br />The signature is valid if and only if the procedure returns.<br /><br />Stack inputs: `[PK, MSG, ...]`<br />Advice stack inputs: `[SIGNATURE]`<br />Outputs: `[...]`<br /><br />Where `PK` is the hash of the public key and `MSG` is the hash of the message, and `SIGNATURE` is the signature being verified. Both hashes are expected to be computed using the native VM hash.<br /><br />|
...
| Procedure                          | Description |
|------------------------------------|-------------|
| verify                             | High-level signature verification. Verifies an Ed25519 EdDSA signature given a public key commitment and the original message. The public key and signature are provided via the advice stack.<br /><br />**Stack inputs:** `[PK_COMM, MSG, ...]`<br />**Advice stack inputs:** `[PK[8], SIG[16], ...]`<br />**Outputs:** `[...]`<br /><br />Where `PK_COMM` is the native hash of the 32-byte Ed25519 public key, `MSG` is a message word serialized as eight u32 limbs before host verification, `PK[8]` is the 32-byte public key packed as 8 field elements, and `SIG[16]` is the 64-byte signature packed as 16 field elements.<br /><br />The procedure traps if the public key does not hash to `PK_COMM` or if the signature is invalid. |
