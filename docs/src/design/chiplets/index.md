...
The execution trace of the Chiplets module is generated by stacking the execution traces of each of its chiplet components. Because each chiplet is expected to generate significantly fewer trace rows than the Core trace components, stacking them enables the same functionality without adding as many columns to the execution trace.
...
The result is an execution trace of 24 trace columns: 23 shared chiplet cells and the chiplet row counter. The last shared cell is row-local: bitwise rows use it as `stream_mode`, and controller rows use it as their Merkle/padding discriminator. The widest overlay is the AEAD stream path, which reuses the bitwise region and extends through `chiplets[2..22]`.
...
The order in which the chiplets are stacked is determined by their selector algebra, row
alignment, trace width, and constraint degree. The hasher controller comes first and is padded to
an 8-row boundary so the following bitwise region can host AEAD stream entries without phase
padding. The remaining chiplets are ordered so higher-degree regions use lower-degree selector
flags.
...
| Normal bitwise | one row per AND/XOR operation | `chiplets[2..15]` | `s_ctrl = 0`, `s1 = 0`, `stream_mode = 0` |
| AEAD stream | 8-row stream entries | `chiplets[2..22]` plus shared mode cell | `s_ctrl = 0`, `s1 = 0`, `stream_mode = 1` |
...
**In the bitwise chiplet:** normal bitwise operations are one-row checks. AEAD stream rows use
the same selector region with `stream_mode = 1`; their internal transitions are confined to the
8-row stream schedule.
...
| Byte-pair lookup messages | AND8, BlakeG rotation-contribution, and range-check lookups against the byte-pair preprocessed table. |
...
- AEAD stream rows are selected by `s0 * (1 - s1) * stream_mode`.
...
| AEAD stream | `s0 * (1 - s1) * stream_mode` |
| Memory | `s0 * s1 * (1 - s2)` |
| ACE | `s0 * s1 * s2 * (1 - s3)` |
...
- **AEAD stream links:** Core requests, BlakeG-XOF input/output messages, stream rows, memory
  messages, and byte-pair checks bind the stream encryption path.
