...
To keep the degree of the constraints low, the operation uses all $6$ helper
registers and the first $10$ elements of the next stack row. These scratch cells
carry the fold2 intermediates, coset flags, $poe^2$, selected $\tau$ factor,
advanced layer pointer, copied folded position, and $poe^4$. The constraints
enforce one active coset flag, compute $x = poe \cdot \tau$, check $pe$ against
the row element selected by the bit-reversed coset, and bind the next-layer
state. After the operation executes, the top $10$ stack elements are scratch
outputs.
