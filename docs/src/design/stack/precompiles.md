...
| `PrecompileTranscript` | A linear hash tree over Poseidon2 that produces a rolling digest of every precompile request: `state' = Poseidon2::merge(state, STMNT)`. The state is itself a complete digest at every step — no separate finalization step is required. The verifier reconstructs the same transcript by re‑evaluating each request and recording its commitment. |
...
4. **`sys::log_precompile_request` folds the commitment into the transcript** – The wrapper calls the helper with `[COMM, TAG, ...]` on top of the stack. The helper:
   ...
   - Seats `STMNT` at stack[4..8] (the HPERM rate1 slots) by pushing one zero word above it, then invokes the `log_precompile` opcode, which folds `STMNT` into the rolling transcript state via `STATE_NEW = Poseidon2::merge(STATE_PREV, STMNT)`. `STATE_PREV` is supplied non‑deterministically via helper registers.
...
8. **No separate finalization step** – Because the fold is a 2‑to‑1 hash (`merge(state, STMNT)`), the state is itself a complete digest at every step. The transcript digest is just the final state — no extra permutation is required.
...
- `log_precompile` stack effect: `[_, STMNT, _, ...] -> [STATE_NEW, OUT_RATE1, OUT_CAP, ...]`
  where `Poseidon2([STATE_PREV, STMNT, ZERO]) = [STATE_NEW, OUT_RATE1, OUT_CAP]` and `STATE_PREV`
  is supplied non‑deterministically via the user op helper registers. `STMNT` lives at
  stack[4..8] so its bus message lanes share with HPERM's rate1.
