...
impl CurveOp {
    ...
    fn node_type(self) -> NodeType {
        match self {
            ...
            Self::Msm { n, .. } => NodeType::PairList(n),
        }
    }
}
...
mod tests {
    ...
    fn assert_invalid_node(result: Result<Node, PrecompileError>) {
        let Err(error) = result else {
            panic!("expected invalid node");
        };
        assert!(
            matches!(error.root(), PrecompileError::InvalidNode),
            "expected invalid node, got {error:?}",
        );
    }
    ...
    fn decode_uses_curve_for_value_and_msm_tags() {
        ...
        assert_eq!(
            ...
            Some(NodeType::PairList(NonZeroU32::MIN))
        );
        assert_eq!(
            precompile.decode(
                CurvePrecompile::msm_tag(CurveId::Secp256k1, NonZeroU32::new(2).unwrap()).args()
            ),
            Some(NodeType::PairList(NonZeroU32::new(2).unwrap()))
        );
    ...
    fn msm_rejects_pair_count_mismatch() {
        ...
        assert_invalid_node(evaluate(&mut state, node));
    }
