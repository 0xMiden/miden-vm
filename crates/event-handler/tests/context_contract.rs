...
impl EventContextProvider for FakeProvider {
    ...
    fn read_memory(
        ...
        self.read(self.current_memory(), start, output)
    }
    ...
    fn read_memory_root(
        ...
        self.read(&self.root_memory, start, output)
    }
...
impl FakeProvider {
    ...
    fn read(
        ...
        for offset in 0..output.len() {
            ...
            let value = memory.get(&address).copied().unwrap_or(Felt::ZERO);
            pending.push(value);
        }
...
#[test]
fn memory_reads_honor_context_zero_fill_and_preserve_buffers_on_error() {
    let (provider, ..) = fixture();
    ...
    assert_eq!(context.memory_value(4).unwrap(), felt(21));
    assert_eq!(context.memory_value_root(4).unwrap(), felt(31));
    assert_eq!(context.memory_value(u64::from(u32::MAX)).unwrap(), felt(25));
    assert_eq!(context.memory_value(8).unwrap(), Felt::ZERO);
    assert_eq!(context.memory_value_root(8).unwrap(), Felt::ZERO);
    assert_eq!(
    ...
    assert_eq!(
        ...
        Word::new([felt(21), felt(22), felt(23), felt(24)])
    );
    assert_eq!(
        context.memory_word_root(4).unwrap(),
        Word::new([felt(31), felt(32), felt(33), felt(34)])
    );
    assert_eq!(context.memory_word(5), Err(EventContextError::UnalignedWord { address: 5 }));
    assert_eq!(context.memory_word(8).unwrap(), Word::default());
    assert_eq!(context.memory_word_root(8).unwrap(), Word::default());
    assert_eq!(
        context.memory_word(u64::from(u32::MAX) + 1),
        Err(EventContextError::AddressOutOfBounds { address: u64::from(u32::MAX) + 1 })
    );

    // Reads agree for untouched and partially written words, including the last word.
    let last_word = u64::from(u32::MAX) - 3;
    assert_eq!(
        context.memory_word(last_word).unwrap(),
        Word::new([Felt::ZERO, Felt::ZERO, Felt::ZERO, felt(25)])
    );
    assert_eq!(context.memory_word_root(last_word).unwrap(), Word::default());
    for start in [0, 4, 8, last_word] {
        let current: Vec<_> = (start..start + 4)
            .map(|address| context.memory_value(address).unwrap())
            .collect();
        let root: Vec<_> = (start..start + 4)
            .map(|address| context.memory_value_root(address).unwrap())
            .collect();
        assert_eq!(context.memory_word(start).unwrap().as_elements(), current.as_slice());
        assert_eq!(context.memory_word_root(start).unwrap().as_elements(), root.as_slice());
        assert_eq!(context.memory_slice(start, 4).unwrap(), current);
        assert_eq!(context.memory_slice_root(start, 4).unwrap(), root);
    }
...
fn provider_errors_after_successful_reads_preserve_current_and_root_buffers() {
    let (mut provider, ..) = fixture();
    // The first element is readable; the second returns a provider error.
...
fn unified_handlers_preserve_pending_output_across_borrows_and_child_imports() {
    ...
    #[allow(clippy::unnecessary_wraps)] // Function handlers must match the fallible trait signature.
    fn handler(
