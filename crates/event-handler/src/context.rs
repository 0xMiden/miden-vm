...
/// Memory reads return zero for unwritten cells. Providers can fill bulk output directly without
...
pub trait EventContextProvider: Sync {
    ...
    /// Reads memory in the current execution context, with unwritten cells zero-filled.
    fn read_memory(
        &self,
        start: MemoryAddress,
        output: &mut [Felt],
    ...
    /// Reads memory in the root execution context, with unwritten cells zero-filled.
    fn read_memory_root(
        &self,
        start: MemoryAddress,
        output: &mut [Felt],
...
///
/// Operand-stack reads extend with zero beyond the tracked depth. Memory reads return zero for
/// unwritten cells, consistently for values, words, and ranges in either execution context.
/// Advice reads require the requested data to exist; missing advice is not implicitly zero.
#[derive(Clone, Copy)]
...
impl<'a> EventContext<'a> {
    ...
    /// Returns the tracked operand-stack depth visible to the handler.
    ///
    /// The dispatch envelope is excluded: one element for an event and two for a trace. This is
    /// bookkeeping, not a read boundary: positions beyond this depth contain zero.
    pub fn stack_depth(&self) -> u32 {
        self.provider.stack_depth().saturating_sub(self.stack_prefix_len())
    }

    /// Returns a handler-visible operand-stack element, zero-extending beyond the tracked depth.
    ...
    /// Allocates a snapshot of the tracked operand-stack elements visible to the handler, top
    /// first.
    ///
    /// The zero extension beyond [`Self::stack_depth`] is not included.
    ...
    /// Returns a memory value from the current execution context. Unwritten cells contain zero.
    ///
    /// The read fails if `address` is outside the VM's `u32` address space.
    pub fn memory_value(&self, address: u64) -> Result<Felt, EventContextError> {
        self.memory_array_from::<1>(MemoryScope::Current, address).map(|[value]| value)
    }

    /// Returns a memory value from the root execution context. Unwritten cells contain zero.
    ///
    /// The read fails if `address` is outside the VM's `u32` address space.
    pub fn memory_value_root(&self, address: u64) -> Result<Felt, EventContextError> {
        self.memory_array_from::<1>(MemoryScope::Root, address).map(|[value]| value)
    }

    /// Returns an aligned memory word from the current execution context, with unwritten cells
    /// zero.
    ///
    /// The read fails if `address` is outside the VM's `u32` address space or is not divisible by
    /// four.
    pub fn memory_word(&self, address: u64) -> Result<Word, EventContextError> {
        self.memory_array_from::<WORD_SIZE>(MemoryScope::Current, address)
            .map(Word::new)
    }

    /// Returns an aligned memory word from the root execution context, with unwritten cells zero.
    ///
    /// The read fails if `address` is outside the VM's `u32` address space or is not divisible by
    /// four.
    pub fn memory_word_root(&self, address: u64) -> Result<Word, EventContextError> {
        self.memory_array_from::<WORD_SIZE>(MemoryScope::Root, address).map(Word::new)
    }

    /// Reads a memory range from the current execution context into `output`.
    ///
    /// Unwritten cells contain zero. Address validation occurs before the provider is
    ...
    ///
    /// Unwritten cells contain zero. The read fails if the address range is invalid.
    ...
    ///
    /// Unwritten cells contain zero. The read fails if the range is reversed or
    ...
    /// Allocates a sparse snapshot of stored memory words in the current execution context.
    ///
    /// Each stored word contributes all four cells, including zero-valued neighbours of a write.
    /// Untouched words are omitted, although reads of their cells return zero.
    pub fn memory_snapshot(&self) -> Vec<(MemoryAddress, Felt)> {
        self.provider.memory_snapshot()
    }

    /// Allocates a sparse snapshot of stored memory words in the root execution context.
    ///
    /// Each stored word contributes all four cells, including zero-valued neighbours of a write.
    /// Untouched words are omitted, although reads of their cells return zero.
    ...
    fn memory_array_from<const N: usize>(
        ...
        scope: MemoryScope,
        address: u64,
    ) -> Result<[Felt; N], EventContextError> {
        ...
        let mut elements = [ZERO; N];
        scope.read(self.provider, memory_address, &mut elements)?;
        Ok(elements)
    }
    ...
    fn read_memory_from(
        ...
        scope.read(self.provider, check_memory_address(start)?, output)
    }
...
impl MemoryScope {
    fn read(
        ...
        match self {
            Self::Current => provider.read_memory(start, output),
            Self::Root => provider.read_memory_root(start, output),
        }
    }
}
