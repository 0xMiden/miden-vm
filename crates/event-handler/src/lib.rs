...
//! address space and defines value, word, count-based slice, and half-open range reads around one
//! canonical strict buffer read. Execution adapters may override the scalar and word fast paths
//! while preserving that contract. Context reads report processor-independent
//! [`EventContextError`] values.
//!
...
pub use context::{
    BuiltinEventContext, BuiltinEventContextProvider, EventContext, EventContextProvider,
    Invocation, InvocationKind, MerkleReadError,
};
