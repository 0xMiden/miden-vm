...
pub enum InputKey {
    ...
    ///
    /// The index is the AIR's semantic instance index, not its proof-order position.
    /// Only present in `is_multi_air = true`.
    IsFirstAir(usize),
    /// Per-AIR lifted last-row selector. The index is the AIR's semantic instance index.
    IsLastAir(usize),
