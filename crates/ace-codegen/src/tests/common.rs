...
use miden_crypto::{
    ...
    stark::dft::{Radix2DitParallel, TwoAdicSubgroupDft},
};
    ...
    EF: ExtensionField<F>,
{
    match expr {
        ...
        SymExpr::IsFirstRow => inputs[layout.index(InputKey::IsFirst).unwrap()],
        SymExpr::IsLastRow => inputs[layout.index(InputKey::IsLast).unwrap()],
        SymExpr::IsTransition => inputs[layout.index(InputKey::IsTransition).unwrap()],
