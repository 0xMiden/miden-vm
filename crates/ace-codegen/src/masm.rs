...
/// one source.
pub fn render_masm_constraints_eval(config: &MasmConstraintsEvalConfig<'_>) -> String {
    assert!(config.stream_len.is_multiple_of(8), "ACE stream must be 8-felt aligned");

    let stream_blocks = config.stream_len / 8;
    let stream_felts = u32::try_from(config.stream_len)
        .expect("ACE stream length must fit in Eidos's u32 length binding");
...
mod tests {
    ...
    fn fold_coefficient_staging_matches_the_relation_read_layout() {
        let staged = render_masm_constraints_eval(&config(true));
        ...
        let bare = render_masm_constraints_eval(&config(false));
        assert!(
    ...
    #[test]
    #[should_panic(expected = "ACE stream must be 8-felt aligned")]
    fn a_misaligned_stream_is_refused() {
        let mut config = config(true);
        config.stream_len = 60;
        render_masm_constraints_eval(&config);
    }
}
