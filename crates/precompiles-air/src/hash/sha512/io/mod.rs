...
pub(super) fn eval_main_rows<AB: LiftedAirBuilder<F = Felt>>(
    ...
    // Byte 111 decides whether the marker already fits before the 128-bit length suffix.
    builder.assert_zero(
        act.clone()
    ...
    // Range16(word_hi), together with the compressor's u32 halves, prevents an alias at 8*n+p.
    builder.assert_zero(
        act.clone()
            * end.clone()
