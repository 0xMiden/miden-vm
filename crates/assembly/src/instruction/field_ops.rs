...
///
/// VM cycles: 11 cycles
pub fn assertw(span_builder: &mut BasicBlockBuilder<'_>, err_code: Felt) {
...
pub fn add_imm(span_builder: &mut BasicBlockBuilder<'_>, imm: Felt) {
    if imm == ZERO {
...
/// - if imm = 0: NOOP
/// - otherwise: PUSH(-imm) ADD
pub fn sub_imm(span_builder: &mut BasicBlockBuilder<'_>, imm: Felt) {
    if imm == ZERO {
        span_builder.push_op(Noop);
    } else {
        span_builder.push_ops([Push(-imm), Add]);
...
pub fn mul_imm(span_builder: &mut BasicBlockBuilder<'_>, imm: Felt) {
    if imm == ZERO {
...
///
/// VM cycles: 16 cycles
pub fn pow2(span_builder: &mut BasicBlockBuilder<'_>) {
...
///
/// VM cycles: 16 cycles
pub fn append_pow2_op(span_builder: &mut BasicBlockBuilder<'_>) {
...
/// - pow = 6: 10 cycles
/// - pow = 7: 12 cycles
fn perform_exp_for_small_power(span_builder: &mut BasicBlockBuilder<'_>, pow: u64) {
    match pow {
        0 => {
...
pub fn ilog2(block_builder: &mut BasicBlockBuilder<'_>) {
...
pub fn eq_imm(span_builder: &mut BasicBlockBuilder<'_>, imm: Felt) {
    if imm == ZERO {
...
pub fn neq_imm(span_builder: &mut BasicBlockBuilder<'_>, imm: Felt) {
    if imm == ZERO {
...
pub fn eqw(span_builder: &mut BasicBlockBuilder<'_>) {
    span_builder.push_ops([
...
pub fn lt(span_builder: &mut BasicBlockBuilder<'_>) {
...
///
/// This operation takes 15 VM cycles.
pub fn lte(span_builder: &mut BasicBlockBuilder<'_>) {
    // Split both elements into high and low bits
    // 3 cycles
    split_elements(span_builder);
...
///
/// This operation takes 15 VM cycles.
pub fn gt(span_builder: &mut BasicBlockBuilder<'_>) {
    // Split both elements into high and low bits
    // 3 cycles
    split_elements(span_builder);
...
pub fn gte(span_builder: &mut BasicBlockBuilder<'_>) {
    // Split both elements into high and low bits
    // 3 cycles
    split_elements(span_builder);
...
///
/// Vm cycles: 5
pub fn is_odd(span_builder: &mut BasicBlockBuilder<'_>) {
...
fn split_elements(span_builder: &mut BasicBlockBuilder<'_>) {
...
///
/// This operation takes 3 cycles.
fn check_lt_and_eq(span_builder: &mut BasicBlockBuilder<'_>) {
    // calculate a - b
...
fn check_lt_high_bits(span_builder: &mut BasicBlockBuilder<'_>) {
...
///
/// This operation takes 3 cycles.
fn check_lt(span_builder: &mut BasicBlockBuilder<'_>) {
    // calculate a - b
    // stack: [b, a, ...] => [underflow_flag, result, ...]
    span_builder.push_op(U32sub);
...
fn set_result(span_builder: &mut BasicBlockBuilder<'_>) {
...
fn check_lte(span_builder: &mut BasicBlockBuilder<'_>) {
    // calculate a - b
    // stack: [b, a, ...] => [underflow_flag, result, ...]
    span_builder.push_op(U32sub);
...
fn check_gt_high_bits(span_builder: &mut BasicBlockBuilder<'_>) {
