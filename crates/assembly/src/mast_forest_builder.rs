...
mod tests {
    ...
    fn test_merge_basic_blocks_preserves_decorator_links_with_padding() {
        ...
        let block1_id = builder.ensure_block(block1_ops, block1_decorators, vec![], vec![]).unwrap();
        ...
        let block2_id = builder.ensure_block(block2_ops, block2_decorators, vec![], vec![]).unwrap();
