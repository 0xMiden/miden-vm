...
impl Assembler {
    ...
    fn verify_exported_signature_type_visibility(
        ...
        for module_index in module_indices.iter().copied() {
            ...
            for symbol in module.symbols() {
                ...
                let (signature_module, signature) = match symbol.item() {
                    SymbolItem::Procedure(proc) => {
                        let proc = proc.borrow();
                        (module_index, proc.signature().cloned())
                    },
                    SymbolItem::Alias { alias, .. } => {
                        let context = SymbolResolutionContext {
                        ...
                        let resolution =
                            resolver.resolve_alias_target(&context, alias).map_err(Report::from)?;

                        let Some(target_gid) = resolution.into_global_id() else {
                            continue;
                        };
                        let signature = match self.linker[target_gid].item() {
                            SymbolItem::Procedure(proc) => {
                                let proc = proc.borrow();
                ...
                self.verify_exported_signature(&resolver, signature_module, signature.as_ref())?;
            }
        }
    ...
    pub(crate) fn assemble_executable_modules(
        ...
        let support_module_indices = self.linker.link_modules(support_modules)?;
        self.verify_exported_signature_type_visibility(&support_module_indices)?;

        // Recompute graph with executable module, and start compiling
        let module_index = self.linker.link([program])?[0];
        self.verify_exported_signature_type_visibility(&[module_index])?;
