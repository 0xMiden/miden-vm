...
#[test]
fn exported_executable_support_module_signature_with_cross_module_private_alias_dependency_is_rejected()
-> TestResult {
    let context = TestContext::default();
    ...
    let program = Module::parse(
        ...
        source_file!(
            ...
            begin
                push.0 exec.cycle::module_b::b_proc
            end
        "#
        ),
        context.source_manager(),
    ...
    let err = match Assembler::new(context.source_manager()).assemble_executable_modules(
        "program".into(),
        program,
        [module_a, module_b],
    ) {
        Ok(_) => panic!(
            "expected executable support module signature with private alias dependency to be rejected"
        ),
        Err(err) => err,
    };
    assert_diagnostic!(&err, "private type in exported procedure signature");
