...
impl pubgrub::VersionSet for VersionSet {
    ...
    fn contains(&self, v: &Self::V) -> bool {
        // The semver range must always cover the version's semver component.
        if !self.range.contains(&v.version) {
            return false;
        }
        // When the set has a digest filter the version must also carry a matching digest.
        if self.digests.is_empty() {
            true
        } else {
            v.digest.as_ref().is_some_and(|d| self.digests.contains(d))
        }
    }
}
