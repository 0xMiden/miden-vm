//! Fixed-point arithmetic underpinning every soundness quantity in this crate.
//!
//! Security is carried as `−log2(error)` in 16-fractional-bit fixed point. The representation is
//! not a convenience: the MASM recursive verifier grades proofs with the same formulas and has no
//! floating-point arithmetic, so every step here is expressible in integer operations. The
//! `#[cfg(test)]` blocks pin each routine against its real-valued counterpart.
//!
//! Rounding is always chosen so the resulting security level is *understated* rather than
//! overstated: logarithms that are added round down, logarithms that are subtracted round up.
...
///
/// Appears in the random-words cutoff of <https://eprint.iacr.org/2025/2010> section 1.5.
...
///
/// This is the "random words" rate of <https://eprint.iacr.org/2025/2010> section 1.5:
///
/// ```text
...
mod tests {
    ...
    /// directions the round budget relies on to stay conservative.
    #[test]
    fn log2_brackets_the_true_logarithm() {
        ...
        while x < u64::MAX / 3 {
            for candidate in [x, x + 1, x * 3 / 2] {
                ...
                assert!(floor <= truth + 1.0, "floor_log2({candidate}) overstates log2");
                assert!(truth - floor < 2.0, "floor_log2({candidate}) is more than 1 ulp low");
                assert!(ceil + 1.0 >= truth, "ceil_log2({candidate}) understates log2");
                assert!(ceil - truth < 2.0, "ceil_log2({candidate}) is more than 1 ulp high");
            }
            x *= 2;
        }
    }
