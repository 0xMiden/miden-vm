...
use miden_processor::{
    DefaultHost, EventError, Program, fast::FastProcessor, parallel::build_trace,
};
...
impl Test {
    ...
    fn assert_trace_with_parallel_trace_generator(
        ...
        let fast_process = FastProcessor::new_with_advice_inputs(&stack_inputs, advice_inputs)
            .with_source_manager(self.source_manager.clone());
        let (_, _, trace_states) =
            fast_process.execute_for_trace_sync(&program, &mut host).unwrap();

        let trace_from_parallel = build_trace(trace_states, program.hash());

        // Helper function to get column name for debugging
        let get_column_name = |col_idx: usize| -> String {
            use miden_air::trace::{
                ...
                decoder::{
                    ...
                    OP_BATCH_FLAGS_OFFSET, OP_BITS_EXTRA_COLS_OFFSET, OP_BITS_OFFSET,
                    OP_INDEX_COL_IDX,
                },
            ...
            match col_idx {
                ...
                i if i >= FN_HASH_OFFSET && i < FN_HASH_OFFSET + 4 => {
                    format!("fn_hash[{}]", i - FN_HASH_OFFSET)
                },

                // Decoder columns
                i if i == DECODER_TRACE_OFFSET + ADDR_COL_IDX => "decoder_addr".to_string(),
                i if i >= DECODER_TRACE_OFFSET + OP_BITS_OFFSET
                    && i < DECODER_TRACE_OFFSET + OP_BITS_OFFSET + NUM_OP_BITS =>
                {
                    format!("op_bits[{}]", i - (DECODER_TRACE_OFFSET + OP_BITS_OFFSET))
                },
                i if i >= DECODER_TRACE_OFFSET + HASHER_STATE_OFFSET
                    && i < DECODER_TRACE_OFFSET + HASHER_STATE_OFFSET + NUM_HASHER_COLUMNS =>
                {
                ...
                i if i >= DECODER_TRACE_OFFSET + OP_BATCH_FLAGS_OFFSET
                    && i < DECODER_TRACE_OFFSET + OP_BATCH_FLAGS_OFFSET + NUM_OP_BATCH_FLAGS =>
                {
                    format!("op_batch_flag[{}]", i - (DECODER_TRACE_OFFSET + OP_BATCH_FLAGS_OFFSET))
                },
                i if i >= DECODER_TRACE_OFFSET + OP_BITS_EXTRA_COLS_OFFSET
                    && i < DECODER_TRACE_OFFSET
                        + OP_BITS_EXTRA_COLS_OFFSET
                        + NUM_OP_BITS_EXTRA_COLS =>
                {
                    format!(
                        "op_bits_extra[{}]",
                        i - (DECODER_TRACE_OFFSET + OP_BITS_EXTRA_COLS_OFFSET)
                    )
                },

                // Stack columns
                i if i >= STACK_TRACE_OFFSET + STACK_TOP_OFFSET
                    && i < STACK_TRACE_OFFSET + STACK_TOP_OFFSET + 16 =>
                {
        ...
        for col_idx in 0..miden_air::trace::RANGE_CHECK_TRACE_OFFSET {
            ...
            if slow_column != parallel_column {
