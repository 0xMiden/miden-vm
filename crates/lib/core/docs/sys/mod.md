...
| log_precompile_request | Logs a precompile commitment and removes the helper words produced by `log_precompile`.<br /><br />Input: `[COMM, TAG, ...]`<br />Output: Stack without the three helper words `[R0, R1, CAP_NEXT]`.<br />Cycles: 1 (plus cost of the underlying `log_precompile` instruction).<br /> |
