...
| eval | Evaluates a curve expression and binds the advised canonical VALUE payload to the input digest.<br />Input:  [POINT_EXPR_DIGEST, ...]<br />Output: [POINT_VALUE_DIGEST, X_OR_TRUE_DIGEST, Y_OR_TRUE_DIGEST, ...]<br />Advice is untrusted. This wrapper re-hashes the advised payload with the registered VALUE frame and<br />logs `eq(EXPR_DIGEST, VALUE_DIGEST)` before returning the value digest and coordinate digests.<br /> |
