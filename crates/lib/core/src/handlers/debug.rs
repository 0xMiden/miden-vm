...
//! (capped) range while `print_mem_all` omits unwritten words.
//!
...
/// showing unwritten cells as zero. `None` denotes an empty range.
///
...
fn write_mem_range<W: fmt::Write>(
    ...
    let items: Vec<_> = (start..=end)
        .map(|addr| {
            ...
            (format!("{addr:#010x}"), Some(value.to_string()))
        })
        .collect();
    write_interval(w, items, None)
}

/// Prints the stored memory cells of the current context, omitting unwritten words.
