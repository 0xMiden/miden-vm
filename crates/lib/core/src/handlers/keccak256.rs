...
//! the hash, and generates a VM-hash commitment that validates the computation was performed
//! correctly.
//!
...
impl PrecompileVerifier for KeccakPrecompile {
    ...
    /// recomputes the Keccak256 hash, and generates a VM-hash commitment with tag
    /// `[event_id, len_bytes, 0, 0]` that validates against the execution trace.
...
impl KeccakFeltDigest {
    ...
    /// Creates a VM-hash commitment of the digest.
...
impl KeccakPreimage {
    ...
    /// Computes the VM-hash commitment to the input data in field element format.
    ...
    /// Computes the precompile commitment with tag `[event_id, len_bytes, 0, 0]`.
    ///
...
mod tests {
    ...
    fn test_keccak_preimage_packing_cases() {
        ...
        for (input, expected_u32) in cases {
            ...
            if input.is_empty() {
                assert_eq!(preimage.input_commitment(), VmHasher::hash_elements(&[]));
            }
        }
    ...
    fn test_keccak_preimage_commitments() {
        ...
        let felts = preimage.as_felts();
        let expected_input_commitment = VmHasher::hash_elements(&felts);
        assert_eq!(preimage.input_commitment(), expected_input_commitment);

        // Test digest commitment
        let digest = preimage.digest();
        let expected_digest_commitment = VmHasher::hash_elements(digest.as_ref());
        ...
        let expected_precompile_commitment = PrecompileCommitment::new(
            ...
            VmHasher::merge(&[preimage.input_commitment(), digest.to_commitment()]),
        );
