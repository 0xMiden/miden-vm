...
impl PrecompileVerifier for KeccakPrecompile {
    ...
    /// recomputes the Keccak256 hash, and produces a [`PrecompileCommitment`] with tag
    /// `[event_id, len_bytes, 0, 0]` and halves `COMM_0 = P2(input)`, `COMM_1 = P2(digest)` that
    /// validates against the execution trace.
...
impl KeccakPreimage {
    ...
    /// Computes the precompile commitment: `(TAG, P2(input), P2(keccak_hash))` with tag
    /// `[event_id, len_bytes, 0, 0]`.
    ///
    /// Generated by the [`PrecompileVerifier`] at verification time and validated against
    /// commitments tracked during execution by the [`EventHandler`]. The transcript folds the
    /// commitment via `Permute(P2(input), P2(keccak_hash), TAG).rate0`, binding input and output.
