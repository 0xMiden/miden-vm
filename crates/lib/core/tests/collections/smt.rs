use miden_core_lib::handlers::smt_peek::{SMT_PEEK_EVENT_NAME, SmtPeekError};
...
/// Regression test: `smt::peek` must reject advice whose preimage does not hash back to the
/// authenticated leaf node.
#[test]
fn test_smt_peek_rejects_forged_leaf_preimage() {
    const SOURCE: &str = "
        use miden::core::collections::smt
    ...
    let key = LEAVES[0].0;
    let honest_value = LEAVES[0].1;
    let forged_value = word(91, 92, 93, 94);
    assert_ne!(honest_value, forged_value);

    let smt = build_smt_from_pairs(&[(key, honest_value)]);
    let root = smt.root();
    let leaf_hash = smt.leaves().next().unwrap().1.hash();

    let store = MerkleStore::from(&smt);
    let advice_map = vec![(leaf_hash, build_leaf_advice_value(&[(key, forged_value)]))];

    let mut initial_stack = Vec::new();
    push_word(&mut initial_stack, &root);
    push_word(&mut initial_stack, &key);

    let err = build_test!(SOURCE, &initial_stack, &[], store, advice_map)
        .execute()
        .expect_err("forged SMT leaf preimage must fail");

    match err {
        miden_processor::ExecutionError::EventError { error, .. } => {
            assert!(
                matches!(
                    error.downcast_ref::<SmtPeekError>(),
                    Some(SmtPeekError::SmtNodePreimageMismatch { .. })
                ),
                "unexpected SMT peek error: {error}"
            );
        },
...
/// Regression test: `smt::peek` must reject advice for a leaf whose keys map to a different
/// leaf index than the queried key.
#[test]
fn test_smt_peek_rejects_leaf_preimage_for_wrong_index() {
    ...
    let query_key = word(10, 11, 12, 0xaaaa);
    let foreign_key = word(20, 21, 22, 0xbbbb);
    let foreign_value = word(31, 32, 33, 34);
    assert_ne!(query_key[3], foreign_key[3]);

    let leaf_elements = build_leaf_advice_value(&[(foreign_key, foreign_value)]);
    ...
    let root = store
        .set_node(
            Smt::new().root(),
            NodeIndex::new(SMT_DEPTH, query_key[3].as_canonical_u64()).unwrap(),
            leaf_hash,
        )
        .unwrap()
        .root;
    let advice_map = vec![(leaf_hash, leaf_elements)];

    let mut initial_stack = Vec::new();
    push_word(&mut initial_stack, &root);
    push_word(&mut initial_stack, &query_key);

    let err = build_test!(SOURCE, &initial_stack, &[], store, advice_map)
        .execute()
        .expect_err("SMT leaf preimage from a different index must fail");
    ...
    match err {
        miden_processor::ExecutionError::EventError { error, .. } => {
            assert!(
                matches!(
                    error.downcast_ref::<SmtPeekError>(),
                    Some(SmtPeekError::SmtLeafIndexMismatch { .. })
                ),
                "unexpected SMT peek error: {error}"
            );
        },
...
/// Regression test: `smt::peek` must reject a Merkle store that routes the requested root and
/// leaf index to a valid but unauthenticated leaf from another tree.
#[test]
fn test_smt_peek_rejects_poisoned_merkle_store_leaf() {
    use miden_utils_testing::crypto::InnerNodeInfo;
    ...
    let key = word(101, 102, 103, 42);
    let honest_value = word(1, 2, 3, 4);
    let forged_value = word(9, 10, 11, 12);
    assert_ne!(honest_value, forged_value);

    let smt_real = build_smt_from_pairs(&[(key, honest_value)]);
    let root_real = smt_real.root();
    let smt_fake = build_smt_from_pairs(&[(key, forged_value)]);
    let root_fake = smt_fake.root();
    assert_ne!(root_real, root_fake);
    ...
    let fake_root_entry = store
        .inner_nodes()
        .find(|node| node.value == root_fake)
        .expect("root_fake should be present after extend");
    store.extend(core::iter::once(InnerNodeInfo {
        value: root_real,
        left: fake_root_entry.left,
    ...
    let err = build_test!(SOURCE, &initial_stack, &[], store, advice_map)
        .execute()
        .expect_err("SMT peek must reject a leaf that is not authenticated by the root");
    ...
    match err {
        miden_processor::ExecutionError::EventError { error, .. } => {
            assert!(
                matches!(
                    error.downcast_ref::<SmtPeekError>(),
                    Some(SmtPeekError::SmtNodeNotAuthenticated { .. })
                ),
                "unexpected SMT peek error: {error}"
            );
        },
