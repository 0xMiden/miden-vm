...
///
/// The five `padw adv_loadw` / `mem_storew_le` pairs and the final two-felt store reproduce the
/// production absorb sequence exactly. Only the transcript observation is replaced by drops, since
/// the transcript is not what this file measures. The sentinels sit under the whole sequence and
