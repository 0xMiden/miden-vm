...
async fn ecdsa_deferred_obligation_is_settled_end_to_end() {
    ...
    let versioned_deferred =
        VersionedProof::new(Verifier::accepted_proof_version(), deferred_proof.clone());
    let outcome = Verifier::new()
        .verify(&claim, &versioned_deferred)
    ...
    let final_proof = VersionedProof::new(Verifier::accepted_proof_version(), final_proof);
    let outcome = Verifier::new()
