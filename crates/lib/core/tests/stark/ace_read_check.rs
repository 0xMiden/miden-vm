...
/// Runs an MVM verifier fixture, checks its ACE inputs, and returns the output stack.
pub(super) fn execute_and_check(test: &Test) -> Vec<Felt> {
    ...
    processor.state().get_stack_state()
}
