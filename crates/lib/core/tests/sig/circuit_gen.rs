...
pub fn build_sig_circuit() -> (Vec<QuadFelt>, Vec<u64>, usize) {
    ...
    for r in 0..STATE_WIDTH {
        for c in 0..STATE_WIDTH {
            mds_const[r][c] = b.add_const(QuadFelt::from(Felt::new_unchecked(mds[r][c])));
        }
    }
...
-> ([[QuadFelt; 8]; STATE_WIDTH], [[QuadFelt; 8]; STATE_WIDTH]) {
    ...
    for col in 0..STATE_WIDTH {
        ...
        for r in 0..NUM_ROUNDS {
            vals1[r] = Felt::new_unchecked(ARK1_U64[r][col]);
            vals2[r] = Felt::new_unchecked(ARK2_U64[r][col]);
        }
...
pub fn encode_for_eval_circuit(
    ...
    for &encoded in operations {
        ...
        stream.push(Felt::new_unchecked(new_encoded));
    }
...
mod tests {
    ...
    fn manual_root_from_inputs(inputs: &[QuadFelt]) -> QuadFelt {
        ...
        for r in 0..STATE_WIDTH {
            ...
            for (c, &wz_c) in wz.iter().enumerate().take(STATE_WIDTH) {
                acc += QuadFelt::from(Felt::new_unchecked(mds[r][c])) * wz_c;
            }
            t[r] = acc;
        }
        ...
        for r in 0..STATE_WIDTH {
            ...
            for (c, &v_c) in v.iter().enumerate().take(STATE_WIDTH) {
                acc += QuadFelt::from(Felt::new_unchecked(mds[r][c])) * v_c;
            }
            w[r] = acc;
        }
    ...
    fn circuit_evaluates_to_zero_on_valid_proof() {
        ...
        let salt: &[miden_signature::Goldilocks; 4] = &proof.salt;
        t.reseed_direct(core::array::from_fn(|i| Felt::from(salt[i])));
        let wc: &[miden_signature::Goldilocks; 4] = &proof.witness_commitment;
        t.reseed_direct(core::array::from_fn(|i| Felt::from(wc[i])));
        assert!(t.check_grind(proof.ali_nonce, stark.grinding.ali));
        ...
        assert!(t.check_grind(proof.ood_nonce, stark.grinding.ood));
        let z = t.sample_ext();
    ...
    fn pow_builder_matches_direct_power() {
        ...
        let mut inputs = vec![QuadFelt::new([Felt::ZERO, Felt::ZERO]); NUM_INPUTS];
        let z = QuadFelt::new([Felt::new_unchecked(123456789), Felt::new_unchecked(987654321)]);
        inputs[ZK_SLOT] = z;
