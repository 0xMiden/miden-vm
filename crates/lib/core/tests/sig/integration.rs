...
use super::{
    ...
    fixtures::{
        ...
        build_fixture_with_message,
    },
    instance_seed_goldilocks, message_to_goldilocks, test_message,
};
...
fn masm_prox_grinding_guard_rejects_bad_nonce() {
    ...
    for offset in 1..1024 {
        ...
        .is_err()
        {
            bad_nonce = Some(proof.prox_nonce);
            break;
        }
    }
    let bad_nonce = bad_nonce.expect("expected nearby PROX nonce to fail grinding");

    let mut data = fixture.data.clone();
    data.advice_stack[PROX_NONCE_ADVICE_INDEX] = bad_nonce;
...
#[test]
fn masm_ood_grinding_guard_rejects_bad_nonce() {
    // Advice order: salt(4), witness_com(4), ali_nonce(1), quotient_com(4), ood_nonce.
    const OOD_NONCE_ADVICE_INDEX: usize = 4 + 4 + 1 + 4;

    let fixture = build_fixture(b"masm-bad-ood-nonce", 1013);
    let mut bad_nonce = None;
    for offset in 1..64 {
        let mut proof = fixture.proof.clone();
        ...
        .is_err()
        {
            bad_nonce = Some(proof.ood_nonce);
            break;
        }
    }
...
#[test]
fn masm_ood_domain_guard_rejects_trace_domain_point() {
    let source = "
        use miden::core::sig
        begin
            push.0.1
            exec.sig::assert_sig_ood_not_in_trace_domain
        end
    ";

    let test = build_test!(source);
    expect_assert_error_code_from_msg!(test, "OOD point z lies in the trace domain");
}
