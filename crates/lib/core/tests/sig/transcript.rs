...
impl SigTranscript {
    /// Initialize: hperm(R1=msg, R2=pk, C=instance_seed), keeping the full
    /// permuted state (no rate zeroing — R2' carries entropy preserved by the
    /// next reseed_direct). Capacity then holds PROOF_SEED.
...
mod tests {
    ...
    fn instance_seed_matches_masm_constants() {
        ...
        for (i, &relation_val) in relation_vals.iter().enumerate() {
            let name = format!("SIG_RELATION_DIGEST_{}", i);
            let from_constants = parse_masm_const(constants, &name)
                .unwrap_or_else(|| panic!("missing {name} in sig/constants.masm"));

            assert_eq!(
                from_constants, relation_val,
                "{name} mismatch between MASM constants and relation digest"
            );
        }
