...
///
/// Version bumps are additive only: a newer version may add host functions, but must
/// not change or remove existing ones. Hosts therefore accept every declared version from `1` up to
/// their own [`ABI_VERSION`], provided each import exists in the declared revision. Revision 2 adds
...
pub enum Status {
    /// The call succeeded.
    Ok = 0,
    /// A memory or advice-stack range was out of bounds.
    OutOfBounds = 1,
