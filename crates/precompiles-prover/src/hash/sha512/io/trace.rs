...
use miden_precompiles_air::hash::sha512::io::{self, program::IV};

use crate::{
    hash::sha512::compression::Sha512CompressionRequires,
...
pub struct Sha512IoRequires {
    invocations: Vec<Invocation>,
}

impl Sha512IoRequires {
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub(crate) fn add_consumers(&mut self, invocation: u32, consumers: ProvideMult) {
        ...
        invocation.out_mult = invocation
            ...
            .expect("too many SHA-512 claim consumers");
    }
    ...
    pub fn require(
        &mut self,
        input: &[u8],
        compression: &mut Sha512CompressionRequires,
        eidos: &mut EidosRequires,
    ) -> Sha512IoOutput {
        let len = u32::try_from(input.len()).expect("SHA-512 input length exceeds u32");
        ...
        let padded_len = padded
            ...
            .checked_mul(128)
            .expect("SHA-512 input length overflow");
        padded.resize(padded_len - 16, 0);
        padded.extend_from_slice(&(u128::from(len) * 8).to_be_bytes());
        ...
        let blocks = padded
            ...
            .map(|bytes| {
                let words = core::array::from_fn(|i| {
                    u64::from_be_bytes(bytes[i * 8..i * 8 + 8].try_into().expect("eight-byte word"))
                });
            ...
            .collect();
        let digest = core::array::from_fn(|i| state[i / 8].to_be_bytes()[i % 8]);
        ...
        let node_eidos = eidos.require_one_shot(
            Sha512Precompile::assert_frame(len),
            raw_eidos.digest.0,
            digest_eidos.digest.0,
        );
        eidos.require_digest(node_eidos.digest).expect("recorded assertion digest");
        let h_sha512 = node_eidos.digest;
        let invocation =
            u32::try_from(self.invocations.len()).expect("SHA-512 invocation count exceeds u32");
        ...
        Sha512IoOutput { digest, h_sha512, invocation }
    }
}
...
pub fn generate_trace_padded_to(
    ...
    let height = requires
        ...
        .expect("SHA-512 IO height overflow")
        .max(io::IO_PERIOD)
        .max(min_height)
    ...
    for (i, row) in values.as_chunks_mut::<{ io::NUM_MAIN_COLS }>().0.iter_mut().enumerate() {
        row[io::COL_BLOCK_ID] =
            Felt::from_u32(u32::try_from(i / io::IO_PERIOD).expect("SHA-512 block id overflow"));
    }
    ...
    for inv in requires.invocations {
        ...
        for (block_index, block) in inv.blocks.iter().enumerate() {
            ...
            for i in 0..io::IO_PERIOD {
                ...
                let offset = block_index * 128 + i * 8;
                ...
                row[io::COL_WORD_LO] = Felt::from_u32(block.words[i] as u32);
                row[io::COL_WORD_HI] = Felt::from_u32((block.words[i] >> 32) as u32);
                if i < 8 {
                    row[io::COL_STATE_LO] = Felt::from_u32(block.state[i] as u32);
                    row[io::COL_STATE_HI] = Felt::from_u32((block.state[i] >> 32) as u32);
                    if final_block {
                        for j in 0..8 {
                ...
                if final_block && i == 15 {
                    bpl.require_range16((block.words[i] >> 32) as u16);
                    ...
                    for (col, value) in io::H_SHA512_COLS.into_iter().zip(inv.node_eidos.digest.0) {
                        row[col] = value;
                    }
                }
