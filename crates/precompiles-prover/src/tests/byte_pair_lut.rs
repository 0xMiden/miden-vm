...
use crate::{
    ...
    primitives::byte_pair_lut::{
        BytePairLutAir, BytePairLutMsg, BytePairLutRequires, BytePairOp, COL_MULT_LOGIC,
        COL_MULT_RANGE16, COL_MULT_ROT7_BEGIN, COL_MULT_ROT12_BEGIN, EidosRotation, NUM_AUX_COLS,
        NUM_MAIN_COLS, NUM_PREPROCESSED_COLS, PRE_A, PRE_AND, PRE_B, PRE_WRAP7, PRE_WRAP12,
        TRACE_HEIGHT, eidos_rotation_contribution, generate_trace, preprocessed_table,
    },
    relations::BusId,
};
...
fn logic_requires_share_one_multiplicity() {
    ...
    assert_eq!(requires.multiplicity_logic(0xab, 0xcd), 1);

    assert_eq!(requires.require(BytePairOp::Xor, 0xab, 0xcd), 0xab ^ 0xcd);
    assert_eq!(requires.multiplicity_logic(0xab, 0xcd), 2);

    assert_eq!(requires.require(BytePairOp::AndNot, 0xab, 0xcd), (!0xabu8) & 0xcd);
    assert_eq!(requires.multiplicity_logic(0xab, 0xcd), 3);
}

#[test]
fn rotation_requires_track_each_family_and_position_independently() {
    let mut requires = BytePairLutRequires::new();
    let a = 0x35;
    let b = 0xa7;

    for rotation in [EidosRotation::Rot12, EidosRotation::Rot7] {
        for byte in 0..4 {
            assert_eq!(
                requires.require_eidos_rotation(rotation, byte, a, b),
                eidos_rotation_contribution(rotation, byte, a, b),
            );
            assert_eq!(requires.multiplicity_rotation(rotation, byte, a, b), 1);
        }
    }

    let trace = generate_trace(requires);
    let row = row(&trace, row_idx(a, b));
    assert_eq!(row[COL_MULT_LOGIC], Felt::ZERO);
    assert_eq!(&row[COL_MULT_ROT12_BEGIN..COL_MULT_ROT12_BEGIN + 4], &[Felt::ONE; 4],);
    assert_eq!(&row[COL_MULT_ROT7_BEGIN..COL_MULT_ROT7_BEGIN + 4], &[Felt::ONE; 4],);
...
fn require_range16_increments_only_its_relation() {
    ...
    assert_eq!(requires.multiplicity_logic(0xcd, 0xab), 0);
    for rotation in [EidosRotation::Rot12, EidosRotation::Rot7] {
        for byte in 0..4 {
            assert_eq!(requires.multiplicity_rotation(rotation, byte, 0xcd, 0xab), 0);
        }
    }
...
fn preprocessed_table_and_derived_rotations_are_correct_for_all_pairs() {
    ...
    for a in 0u16..256 {
        for b in 0u16..256 {
            ...
            assert_eq!(p[PRE_AND], Felt::from(a & b));
            assert_eq!(
                p[PRE_WRAP12],
            ...
            for byte in 0..4 {
                assert_eq!(
                    derived_rot12[byte],
                    Felt::from(eidos_rotation_contribution(EidosRotation::Rot12, byte, a, b)),
                );
                assert_eq!(
                    derived_rot7[byte],
                    Felt::from(eidos_rotation_contribution(EidosRotation::Rot7, byte, a, b)),
                );
            }
        }
    }
...
fn trace_rows_carry_the_aggregated_relation_multiplicities() {
    ...
    requires.require_eidos_rotation(EidosRotation::Rot12, 2, 0x05, 0x03);
    requires.require_eidos_rotation(EidosRotation::Rot7, 3, 0x05, 0x03);
    requires.require_range16(0x0301);

    let trace = generate_trace(requires);

    let one_logic = row(&trace, row_idx(0x01, 0x02));
    assert_eq!(one_logic[COL_MULT_LOGIC], Felt::ONE);
    ...
    assert_eq!(shared[COL_MULT_LOGIC], Felt::from(3u8));
    assert_eq!(shared[COL_MULT_ROT12_BEGIN + 2], Felt::ONE);
    assert_eq!(shared[COL_MULT_ROT7_BEGIN + 3], Felt::ONE);
...
fn build_aux_trace_has_the_declared_shape_and_zero_initial_state() {
    ...
    requires.require(BytePairOp::Xor, 0x05, 0x03);
    requires.require(BytePairOp::AndNot, 0x10, 0x20);
    requires.require_eidos_rotation(EidosRotation::Rot12, 1, 0xab, 0xcd);
