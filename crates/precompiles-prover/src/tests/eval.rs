...
use crate::{
    ...
    transcript::{
        ...
        eval::{
            ...
            COL_IS_AND, COL_IS_EC_CREATE, COL_IS_EC_PAI, COL_IS_IS, COL_IS_MUL, COL_IS_PINNED,
            COL_IS_SUB, COL_IS_UINT_LEAF, COL_IS_UINT_OP, COL_LHS_BEGIN, COL_OUT_MULT,
            COL_PIN_CLAIM_PIN_PTR, COL_PTR, COL_RHS_BEGIN, COL_TAG_ARG0, DIGEST_WIDTH,
...
fn corruption_non_binary_is_add() {
    ...
    // tag (-1 * Add + Sub + Is = 5) so the derived flag's boolean check rejects it.
    let mut row = vec![Felt::ZERO; NUM_MAIN_COLS];
    for col in [COL_ACT, COL_IS_UINT_OP, COL_IS_SUB, COL_IS_IS] {
        row[col] = Felt::ONE;
    }
    row[COL_TAG_ARG0] = Felt::from(5u8);
    row.resize(2 * NUM_MAIN_COLS, Felt::ZERO);
    crate::tests::check_local_inputs(
        TranscriptEvalAir,
        &RowMajorMatrix::new(row, NUM_MAIN_COLS),
