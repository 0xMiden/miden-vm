...
use miden_air::lookup::{
    LookupAir, LookupFractions, LookupMessage, ProverLookupBuilder, build_lookup_fractions,
...
use crate::{
    ...
    session::Session,
    tests::{assert_same_rational_fold, bus_balance::session_stack_residual},
    transcript::{
...
fn shared_assertions_balance_actual_operand_uses() {
    ...
    let mains = traces.mains();
    let eval = mains[4];
    let multiplicity = |hash: EidosDigest| {
        eval.values
            .as_chunks::<NUM_MAIN_COLS>()
            .0
            .iter()
...
fn final_root_with_parent_is_rejected() {
    ...
    let zero = req.zero();
    let root = req.record_and(zero, zero, &mut eidos);
    let _parent = req.record_and(root, root, &mut eidos);
...
fn typed_relation_fold(row: Vec<Felt>, challenges: &Challenges<QuadFelt>) -> (QuadFelt, QuadFelt) {
    ...
    let fractions = build_lookup_fractions(&air, &main, None, &periodic, challenges);
    fold_fractions(fractions_at(&fractions, 0, TYPED_RELATION_COL))
}
...
#[test]
#[should_panic(expected = "constraint not satisfied")]
fn corruption_non_msm_row_has_msm_index() {
    let mut rng = StdRng::seed_from_u64(0xec_15_1d);
    let (mut main, public_root) = build_eval_trace(&mut rng, 3);
    let row = (1..main.height())
        .find(|&row| {
            main.values[row * NUM_MAIN_COLS + COL_ACT] == Felt::ONE
                && main.values[row * NUM_MAIN_COLS + COL_IS_EC_MSM] == Felt::ZERO
        })
        .expect("trace has an active non-MSM row after row zero");

    main.values[row * NUM_MAIN_COLS + COL_MSM_IDX] = Felt::ONE;
    crate::tests::check_local_inputs(TranscriptEvalAir, &main, public_root.as_array().to_vec());
}
