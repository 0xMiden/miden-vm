...
use miden_precompiles_air::hash::sha512::{
    COL_IO_ACT, IO_COLUMNS, IO_ROW_START, NUM_MAIN_COLS, Sha512IoContinuation,
};

use crate::{
    hash::sha512::{
        Sha512Air,
        compression::{self, Sha512CompressionAir, Sha512CompressionRequires},
        io::{self, Sha512IoAir, Sha512IoRequires},
...
fn requirements(lengths: &[usize]) -> (Sha512CompressionRequires, Sha512IoRequires) {
...
#[test]
fn sha512_multiplex_preserves_dense_lookup_demands_and_empty_heights() {
    // Cover no invocations, an empty message, an exact final row, a padded fourth block,
    // and multiple invocations with duplicate messages and shared Eidos spans.
    for lengths in [&[][..], &[0], &[112], &[256], &[129, 1, 2, 1, 0]] {
        ...
        fold_balance(&Sha512CompressionAir, &dense_comp, &challenges, &mut net);
        fold_balance(&Sha512IoAir, &dense_io, &challenges, &mut net);
        for (multiplicity, _) in net.values_mut() {
            *multiplicity = -*multiplicity;
        }
        fold_balance(&Sha512Air, &sparse, &challenges, &mut net);
...
fn sha512_multiplex_rejects_wrong_row_ownership_and_io_witnesses() {
    ...
    for (name, index, value) in [
        ...
        assert!(std::panic::catch_unwind(|| check_local(Sha512Air, &bad)).is_err(), "{name}");
    }
    let mut bad = main;
    bad.values[io_index(0, 15, io::COL_DIGEST_EIDOS)] += Felt::ONE;
    assert!(
        std::panic::catch_unwind(|| check_local(Sha512Air, &bad)).is_err(),
        "digest pointer must be held across IO row 14 -> 15",
    );
}
...
fn continuation(
    ...
    block: usize,
    provide: bool,
) -> Sha512IoContinuation<Felt> {
    let lane = if provide { 15 } else { 0 };
...
#[test]
fn sha512_multiplex_continuation_binds_every_transport_field() {
    let (compression, io) = requirements(&[256]);
    let main = generate_trace(compression, io, &mut BytePairLutRequires::new());
    let challenges = challenges();
    ...
    for column in [
        ...
        let mut bad = main.clone();
        bad.values[io_index(0, 15, column)] += Felt::ONE;
        let changed = continuation(&bad, 0, true).encode(&challenges);
        let mut net = HashMap::new();
        fold_balance(&Sha512Air, &bad, &challenges, &mut net);
...
#[test]
fn sha512_multiplex_endpoints_and_cross_gap_pointers_need_global_closure() {
    let (compression, io) = requirements(&[256]);
    let main = generate_trace(compression, io, &mut BytePairLutRequires::new());
    let challenges = challenges();
    let residual = |main: &RowMajorMatrix<Felt>| {
        let mut net: HashMap<QuadFelt, (Felt, String)> = HashMap::new();
        fold_balance(&Sha512Air, main, &challenges, &mut net);
        net
    };
    let mut bad = main.clone();
    for lane in 0..16 {
