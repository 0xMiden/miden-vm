...
",
    ...
    #[test]
    fn semantic_const_eval_completes_quickly_on_shared_subexpressions() {
        use std::{
            env,
        ...
        fn generate_source(depth: usize) -> String {
        ...
        fn wait_with_timeout(mut child: Child, timeout: Duration) -> std::process::ExitStatus {
            ...
            loop {
                ...
                if start.elapsed() > timeout {
                    let _ = child.kill();
                    return child.wait().expect("child wait must succeed after kill");
                }
                std::thread::sleep(Duration::from_millis(10));
            }
        }

        const CHILD_ENV: &str = "MIDEN_CONST_EVAL_NO_MEMOIZATION_CHILD";
        const TEST_NAME: &str =
            "parser::tests::semantic_const_eval_completes_quickly_on_shared_subexpressions";

        if env::var_os(CHILD_ENV).is_some() {
            // This input is small in bytes, but forces repeated evaluation of the same constants
            // when results are not memoized.
            let depth = 26;
            let source = generate_source(depth);
            let source_manager = Arc::new(DefaultSourceManager::default());
            let parsed = source.parse_with_options(source_manager, ParseOptions::default());
            assert!(parsed.is_ok(), "parsing should succeed");
            return;
        }

        let exe = env::current_exe().expect("current test binary path must be available");
        let listed = Command::new(&exe)
            .arg("--list")
            .arg("--exact")
            .arg(TEST_NAME)
            .output()
            .expect("failed to query child test list");
        assert!(
            listed.status.success() && String::from_utf8_lossy(&listed.stdout).contains(TEST_NAME),
            "child test list did not include '{TEST_NAME}'"
        );
        ...
        let timeout_secs = env::var("MIDEN_CONST_EVAL_TEST_TIMEOUT_SECS")
            .ok()
            .and_then(|value| u64::from_str(value.as_str()).ok())
            .unwrap_or(5);
        // Leave room for process startup/test harness overhead while keeping a strict budget for
        // the pathological dependency graph.
        let status = wait_with_timeout(child, Duration::from_secs(timeout_secs));
        assert!(status.success(), "child was too slow or failed: {status:?}");
    }
}
