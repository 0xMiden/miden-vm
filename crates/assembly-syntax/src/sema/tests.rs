...
#[test]
fn sema_warns_on_unused_private_constants() {
    let context = SyntaxTestContext::new().with_warnings_as_errors(true);
    let error = context
        .parse_module(
            r#"
namespace test
const UNUSED = 1
const USED_BY_CONST = 2
const USED = USED_BY_CONST + 1
const USED_BY_ASSERT = "expected non-zero value"
const USED_BY_ENUM = 4
const USED_BY_REPEAT = 2
pub const EXPORTED = 3
...
proc local
    push.USED
    push.OK
    assert.err=USED_BY_ASSERT
    repeat.USED_BY_REPEAT
        nop
    end
end
"#,
        )
        .expect_err("unused private constant should warn");

    assert_eq!(unused_constant_slices(&error), vec!["UNUSED"]);
}

#[test]
fn sema_skips_unused_private_constants_in_generated_sources() {
    let context = SyntaxTestContext::new().with_warnings_as_errors(true);
    let _module = context
        .parse_module(
            r#"# GENERATED by a tool - do not edit by hand.
namespace test
const UNUSED = 1
proc local
    nop
...
"#,
        ...
        .expect("generated source should not warn about unused private constants");
}

#[test]
fn sema_skips_unused_private_constants_in_declaration_only_modules() {
    let context = SyntaxTestContext::new().with_warnings_as_errors(true);
    let _module = context
        .parse_module(
            r#"
...
"#,
        ...
        .expect("declaration-only modules should not warn about unused private constants");
}

#[test]
fn sema_skips_documented_unused_private_constants() {
    let context = SyntaxTestContext::new().with_warnings_as_errors(true);
    let _module = context
        .parse_module(
            r#"
...
#! Reserved for a layout boundary outside MASM control flow.
const RESERVED = 1
...
"#,
        ...
        .expect("documented private constants should not warn as unused");
}

#[test]
fn sema_skips_advice_map_entry_constants() {
    let context = SyntaxTestContext::new().with_warnings_as_errors(true);
    let _program = context
        .parse_program(
            r#"
...
"#,
        ...
        .expect("advice map entries should not warn as unused constants");
}
