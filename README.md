...
- **Deferred computation.** Miden VM supports [deferred computation](./docs/src/design/deferred/index.md), letting programs defer expensive or non-native operations — signature checks, hash functions, elliptic-curve and big-integer arithmetic — and emit a content-addressed record of what was claimed, committed by a single digest and verified externally. This keeps operations that would be prohibitively expensive to run natively cheap inside the VM while still producing auditable evidence.
