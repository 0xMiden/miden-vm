...
fn end_to_end_register_evaluate_assert_extract() {
    ...
    let canonical = state.evaluate(&schema, *state.get(&mul).unwrap()).unwrap();
...
fn transcript_folds_assertion_digests_in_order() {
    ...
    let a_eq_a = Node::new(Field0Handler::ASSERT_EQ, Payload::binary_op(a, a));
    state.register(&schema, a_eq_a).unwrap();
    ...
    let b_eq_b = Node::new(Field0Handler::ASSERT_EQ, Payload::binary_op(b, b));
    state.register(&schema, b_eq_b).unwrap();
