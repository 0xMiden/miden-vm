...
/// Two capabilities:
///
/// - [`resolve`](Self::resolve) — walk a child digest to its canonical form. The framework reduces
///   the child via `Schema::reduce` and interns both the input and the canonical along the way, so
///   `reduce` reads as a depth-first recursive function: "resolve lhs, resolve rhs, combine."
///   Because chunk-handling schemas canonicalise chunks to a digest-leaf expression, parent
///   op/assertion schemas only ever see expression-shaped canonical forms.
/// - [`intern`](Self::intern) — mint a brand-new canonical node into the DAG mid-`reduce`, and get
///   its digest back so the returned canonical can reference it by child digest. Required for
///   compound canonicals (e.g. a group element whose payload contains coordinate-leaf digests that
///   were just computed). Idempotent by digest; the framework does NOT re-reduce the interned node,
///   so it's the schema's responsibility to pass a node that is already in canonical form (e.g. a
...
pub trait Schema: core::fmt::Debug + Send + Sync {
    ...
    ///   whose payload contains by-digest children) mint those children via `ctx.intern`.
