...
///
/// `AND` nodes hash their single digest pair in the AND domain, `CHUNKS` nodes absorb their blocks
/// under the CHUNKS domain with the logical felt length, and precompile-owned nodes bind their
/// domain tag, complete payload length, and two local arguments into the initial chaining value.
/// The payload is an aligned sequence of complete Eidos blocks.
pub(super) fn hash_payload(tag: Tag, chunks: impl ExactSizeIterator<Item = DataChunk>) -> Digest {
    let payload_len = Node::DATA_CHUNK_FELT_LEN
    ...
    let mut cv = if tag == Tag::CHUNKS {
        Eidos::init_chaining_word(DEFERRED_CHUNKS, payload_len)
    } else {
        assert!(tag.has_canonical_reserved_lane(), "deferred tag reserved lane must be zero");
        assert!(tag.has_canonical_init_values(), "deferred tag values must fit in u32");
        let [arg0, arg1] = tag.args();
        let domain_tag = parse_domain_tag(tag.id()).expect("validated deferred tag must be a u32");
