...
/// registered selector, complete payload length, and two local arguments into the initial
/// chaining value. The payload is an aligned sequence of complete Eidos blocks.
pub(super) fn hash_payload(tag: Tag, chunks: impl ExactSizeIterator<Item = DataChunk>) -> Digest {
    ...
    let mut cv = if tag == Tag::CHUNKS {
        Eidos::init_chaining_word(DEFERRED_CHUNKS_DOMAIN.as_canonical_u64() as u32, payload_len)
    } else {
        ...
        Eidos::init_chaining_word_with_params(
            tag.id().as_canonical_u64() as u32,
