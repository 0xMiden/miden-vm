...
mod tests {
    ...
    use crate::{
        Felt, Word, ZERO,
        deferred::{
            DeferredStateWire, Payload, Precompile, PrecompileRegistry, TRUE_DIGEST, TRUE_INDEX,
            Tag, WireEntry, precompile_id,
        },
    ...
    fn unary_precompiles() -> Arc<PrecompileRegistry> {
        Arc::new(
    ...
    impl UnaryFixture {
        const NAME: &'static str = "unary-fixture";
        ...
        fn node(child: Digest, params: Word) -> Node {
            Node::unary(Self::tag(), child, params).expect("fixture tag is precompile-owned")
        }
    }
    ...
    impl PairListFixture {
        const NAME: &'static str = "state-pair-list-fixture";
    ...
    impl Precompile for PairListFixture {
        ...
        fn decode(&self, args: [Felt; 3]) -> Option<NodeType> {
            ...
            let n = u32::try_from(args[0].as_canonical_u64()).ok()?;
            NodeType::pair_list(n)
        }
    ...
    #[test]
    fn chunks_nodes_self_evaluate_but_are_not_true_statements() {
        let registry = empty_precompiles();
        let mut state = state_with(&registry, usize::MAX);
        let chunks = Node::chunks(preimage_chunks()).unwrap();
        let digest = chunks.digest();

        assert_eq!(state.register(chunks.clone()).unwrap(), digest);
        assert_eq!(state.evaluate_digest(digest).unwrap(), digest);
        assert_eq!(state.get_node(&digest), Some(&chunks));
        assert_eq!(state.get_canonical_digest(digest), Some(digest));

        let root_before = state.root();
        let err = state.log_statement(digest).unwrap_err();
        assert!(matches!(err.root(), PrecompileError::AssertionFailed));
        assert_eq!(state.root(), root_before);
    }

    #[test]
    fn canonical_helpers_return_memoized_results_after_registration() {
        let registry = hash_precompiles();
        let mut state = state_with(&registry, usize::MAX);
        let chunks = preimage_chunks();
        let original = preimage_node(chunks.clone());
        ...
        let digest = state.register(original).unwrap();
        let canonical_digest = canonical.digest();

        assert_eq!(state.get_canonical_digest(digest), Some(canonical_digest));
        assert_eq!(state.get_canonical_node(digest), Some((canonical_digest, &canonical)));
        assert_eq!(state.require_canonical_node(digest).unwrap(), (canonical_digest, &canonical));

        let unknown = dummy_digest(99);
        assert_eq!(state.get_canonical_digest(unknown), None);
        assert_eq!(state.get_canonical_node(unknown), None);
        assert!(matches!(
            state.require_canonical_node(unknown),
            Err(PrecompileError::MissingNode)
        ));
    }

    #[test]
    fn canonical_helpers_are_read_only_and_do_not_evaluate_unmemoized_nodes() {
        let registry = hash_precompiles();
        let mut state = state_with(&registry, usize::MAX);
        let original = preimage_node(preimage_chunks());
        let digest = state.insert_node(original).unwrap();
        let nodes_before = state.nodes().clone();
        let remaining_before = state.remaining_elements();

        assert!(!state.evals.contains_key(&digest));
        assert_eq!(state.get_canonical_digest(digest), None);
        assert_eq!(state.get_canonical_node(digest), None);
        assert!(matches!(
            state.require_canonical_node(digest),
            Err(PrecompileError::MissingNode)
        ));
        assert_eq!(state.nodes(), &nodes_before);
        assert_eq!(state.remaining_elements(), remaining_before);
        assert!(!state.evals.contains_key(&digest));

        // Repeated read-only lookups must still leave the state unchanged and the digest
        // unevaluated.
        assert_eq!(state.get_canonical_digest(digest), None);
        assert_eq!(state.nodes(), &nodes_before);
        assert_eq!(state.remaining_elements(), remaining_before);
    ...
    #[test]
    fn canonical_helpers_treat_dangling_memos_as_missing() {
        let registry = hash_precompiles();
        let mut state = state_with(&registry, usize::MAX);
        let node = digest_node(7);
        let digest = state.insert_node(node).unwrap();
        state.evals.insert(digest, dummy_digest(123));
    ...
    fn register_eagerly_rejects_false_predicate() {
        ...
        let err = state.register(Hash::eq_node(a, b)).unwrap_err();
        assert!(matches!(err.root(), PrecompileError::AssertionFailed));
    }
    ...
    #[test]
    fn unary_wire_round_trip_treats_params_as_literal_payload() {
        let registry = unary_precompiles();
        let mut state = state_with(&registry, usize::MAX);
        let child = state.register(digest_node(5)).unwrap();
        let params = dummy_digest(999);
        assert!(state.get_node(&params).is_none(), "test params must not already be registered");

        let statement = state.register(UnaryFixture::node(child, params)).unwrap();
        let expected_root = state.log_statement(statement).unwrap();
        let wire = state.to_wire().unwrap();

        assert!(
            wire.entries.iter().any(
                |entry| matches!(entry, WireEntry::Unary { child: 1, params: p, .. } if *p == params)
            ),
            "unary wire stores one child index and literal params"
        );

        let rehydrated =
            DeferredState::from_wire(Arc::clone(&registry), &wire, usize::MAX).unwrap();
        assert_eq!(rehydrated.root(), expected_root);
        assert_eq!(rehydrated.to_wire().unwrap(), wire);
        assert!(
            rehydrated.get_node(&params).is_none(),
            "params must not be required or registered as a child digest"
        );
    }

    #[test]
    fn pair_list_wire_round_trip_treats_pairs_as_structural_children() {
        let registry = pair_list_precompiles();
        let mut state = state_with(&registry, usize::MAX);
        let scalar_0 = state.register(digest_node(5)).unwrap();
        let point_0 = state.register(digest_node(6)).unwrap();
        let scalar_1 = state.register(digest_node(7)).unwrap();
        let point_1 = state.register(digest_node(8)).unwrap();

        let statement = state
            .register(
                Node::try_pair_list(
                    PairListFixture::tag(2),
                    vec![(scalar_0, point_0), (scalar_1, point_1)],
                )
                .unwrap(),
            )
            .unwrap();
        ...
        assert!(wire.entries.iter().any(
            |entry| matches!(entry, WireEntry::PairList { pairs, .. } if pairs == &vec![(1, 2), (3, 4)])
        ));
        ...
        assert_eq!(rehydrated.root(), expected_root);
        assert_eq!(rehydrated.to_wire().unwrap(), wire);
        for digest in [scalar_0, point_0, scalar_1, point_1, statement] {
            assert!(rehydrated.get_node(&digest).is_some(), "pair-list child/root must be wired");
        }
    }
