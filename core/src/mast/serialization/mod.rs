    ...
    fn write_into<W: ByteWriter>(&self, target: &mut W) {
        ...
        let mast_node_infos: Vec<MastNodeInfo> = self
            ...
            .map(|(mast_node_id, mast_node)| {
                ...
                let before_decorators =
                    self.debug_info.node_decorator_storage.get_before_decorators(node_id);
                if !before_decorators.is_empty() {
                    before_enter_decorators.push((mast_node_id, before_decorators.to_vec()));
                }

                let after_decorators =
                    self.debug_info.node_decorator_storage.get_after_decorators(node_id);
        ...
        let error_codes: BTreeMap<u64, String> =
            self.debug_info.error_codes.iter().map(|(k, v)| (*k, v.to_string())).collect();
...
impl Deserializable for MastForest {
    fn read_from<R: ByteReader>(source: &mut R) -> Result<Self, DeserializationError> {
        ...
        mast_forest.debug_info.error_codes = error_codes;
