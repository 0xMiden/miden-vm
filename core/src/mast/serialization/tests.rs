...
/// Test that UntrustedMastForest::into_inner bypasses validation.
#[test]
fn test_untrusted_forest_into_inner() {
    let mut forest = MastForest::new();
    let block_id = BasicBlockNodeBuilder::new(vec![Operation::Add], Vec::new())
        .add_to_forest(&mut forest)
        .unwrap();
    ...
    // into_inner should return the forest without validation
    let inner = untrusted.into_inner();
    assert_eq!(forest.num_nodes(), inner.num_nodes());
}
