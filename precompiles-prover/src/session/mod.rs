...
use crate::uint::add::trace::generate_trace as uint_add_trace;
use crate::uint::mul::trace::generate_trace as uint_mul_trace;
use crate::uint::trace::UintPtr;
use crate::uint::trace::generate_trace as uint_trace;
...
impl Session {
    ...
    pub fn finish(mut self, root: Truthy) -> SessionTraces {
        ...
        let add = uint_add_trace(self.uint.add, &mut self.uint.store);
        let mul = uint_mul_trace(self.uint.mul, &mut self.uint.store, &mut self.bpl);
        ...
        let uint = uint_trace(self.uint.store, &mut self.bpl);
        ...
        let ec_add = ec_add_trace(self.ec.add, &mut self.ec.store, &mut self.bpl);
        let (ec_groups, ec) = ec_store_traces(self.ec.store);
