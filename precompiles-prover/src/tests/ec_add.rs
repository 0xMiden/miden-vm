...
//! certificate tuples nothing recorded (the λ-float attack's inv·d ≡ 1
...
use crate::{
    ec::{
        ...
        add::{
            CELL_R, COL_CANCEL, COL_DBL, COL_GEN, COL_MINTS, COL_ONE_PTR, COL_PAI_P, COL_PAI_Q,
            EcGroupAddAir, NUM_MAIN_COLS as ADD_COLS, PERIOD, ROW_RES,
    ...
    for<'a> A: LookupAir<DebugTraceBuilder<'a>>,
{
    ...
    for u in report.unmatched {
        let entry =
            net.entry(u.denom).or_insert_with(|| (Felt::ZERO, type_name::<A>().to_string()));
...
fn ec_add_matches_k256() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecaddc256);
    traces.check();
...
fn generic_add_computes_kat() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecadd001);
    traces.check();
...
fn duplicate_adds_collapse() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecaddded);
    traces.check();
    assert_eq!(stack_residual(&traces.mains(), &mut rng), 0);
}
...
fn double_binds_canonically() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecadd002);
    traces.check();
    assert_eq!(stack_residual(&traces.mains(), &mut rng), 0);
}
...
fn cancel_resolves_to_canonical_pai() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecadd003);
    traces.check();
    assert_eq!(stack_residual(&traces.mains(), &mut rng), 0);
}
...
fn pai_passthroughs_tie_results() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecadd004);
    traces.check();
    assert_eq!(stack_residual(&traces.mains(), &mut rng), 0);
}

#[test]
fn negating_pai_returns_pai() {
    // -∞ = ∞. The helper still records the group-add relation so a
    // future consumer can bind the returned point through EcGroupAdd.
    let mut k1 = k1_stack();
    let (group, neg, pai) = k1.stack.require().neg(k1.pai, 0);
    assert_eq!(group, k1.group);
    assert_eq!(neg, k1.pai);
    assert_eq!(pai, k1.pai);

    let traces = k1.stack.traces();
    assert_eq!(block0_flags(traces.ec_add_main()), [1, 1, 0, 0, 0].map(Felt::from_u32));

    let mut rng = StdRng::seed_from_u64(0xecadd0ff);
    traces.check();
    assert_eq!(stack_residual(&traces.mains(), &mut rng), 0);
}
...
fn ed25519_torsion_doubles_to_pai() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecadd25519);
    traces.check();
    assert_eq!(stack_residual(&traces.mains(), &mut rng), 0);
}
...
fn double_forged_as_generic_unbalances() {
    ...
    // the disequality witness inv·d ≡ 1, which is unrecordable at d = 0,
    // and the rest of the chord certificates
    ...
    let mut rng = StdRng::seed_from_u64(0xecadda01);
    check_ec_add(&forged);
...
fn generic_forged_as_double_unbalances() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecadda02);
    check_ec_add(&forged);
...
fn cancel_forged_on_distinct_x_unbalances() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecadda06);
    check_ec_add(&forged);
...
fn finite_forged_as_pai_unbalances() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecadda03);
    check_ec_add(&forged);
...
fn double_forged_as_cancel_unbalances() {
    ...
    tamper_cell(&mut forged, ROW_RES, CELL_R, pai.addr());
    let mut rng = StdRng::seed_from_u64(0xecadda04);
    check_ec_add(&forged);
...
fn forged_result_ptr_unbalances() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecadda05);
    check_ec_add(&forged);
...
#[test]
fn forged_one_ptr_unbalances() {
    // The inverse certificates target a stored one. Repointing the hosted
    // one ptr at the field modulus leaves local EC-add constraints intact,
    // but the UintVal([1, 0, 0, 0], [0, 0, 0, 0]) consumes no longer match.
    let mut k1 = k1_stack();
    k1.stack.require().add(k1.g_pt, k1.g2_pt, 0);
    let fp_addr = k1.stack.fp.addr();
    let traces = k1.stack.traces();

    let forged = tamper_block0(traces.ec_add_main(), &[(COL_ONE_PTR, fp_addr)]);
    let mut rng = StdRng::seed_from_u64(0xecadda07);
    check_ec_add(&forged);
...
fn cert_point_forged_as_trio_unbalances() {
    ...
    let mut rng = StdRng::seed_from_u64(0xecaddce3);
    let mut mains = traces.mains();
    mains[5] = &forged;
    assert_ne!(stack_residual(&mains, &mut rng), 0);
}
