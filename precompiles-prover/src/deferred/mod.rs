...
/// `AND(TRUE_DIGEST, Keccak256Assert(preimage_chunks(input), digest_chunks(keccak256(input))))`.
pub(crate) fn synthetic_keccak_state(input: &[u8]) -> SyntheticKeccakDeferredState {
    ...
    let input_digest = state
        .register(
            Node::try_data(
                Keccak256Precompile::preimage_tag(len_bytes(input)),
                pack_input_chunks(input),
            )
            .expect("input chunks are non-empty"),
        )
        .expect("VM should register input chunks");
    let expected_digest = state
        .register(
            Node::try_data(Keccak256Precompile::digest_tag(), keccak_digest_chunks(input))
