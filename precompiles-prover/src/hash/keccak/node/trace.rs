...
pub struct KeccakNodeInvocation {
    ...
    /// Downstream consumer count for the `Binding(H_keccak, True, 0, 0)`
...
/// `(rate0 = D[0..4], rate1 = D[4..8], cap = VM Keccak digest data tag)`. Returns the
...
pub fn hash_digest_chunks(d: &[Felt; 8]) -> [Felt; NUM_HASH] {
    ...
    Poseidon2Requires::digest_of(P2Cap::keccak256_digest(), &[(rate0, rate1)]).as_array()
}
...
/// trace where the `Binding(H_keccak, True, 0, 0)` provide fires.
#[derive(Debug, Clone)]
...
impl KeccakNodeRequires {
    ...
    pub fn require(
        ...
        let digest_chunks_out = p2.require_one_shot(P2Cap::keccak256_digest(), d_rate0, d_rate1);
