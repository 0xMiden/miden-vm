...
//!   `bound(β)` on the `p` row, holds through the `q` row (`−(S+1)·q(β)` — the `+1` is
//!   `p = bound + 1`), resets. One register serves both products via the periodic keep gate
//!   [`S_KEEP`].
//! - **`id`**: accumulates every role's contribution; `when_first_row` pins 0, the `c` row's
//!   folded check asserts the block closes to 0.
//!
//! ## Padding
//!
