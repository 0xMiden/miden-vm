...
/// Neg-value cell (boundary only): the point-at-infinity result slot for
/// `EcGroupAdd(group, val_a, val, ∞)`.
pub const COL_NEG_PAI: usize = 33;
...
pub const NUM_MAIN_COLS: usize = 35;
...
impl LiftedAir<Felt, QuadFelt> for EcMsmAir {
    ...
    fn eval<AB: LiftedAirBuilder<F = Felt>>(&self, builder: &mut AB) {
        ...
        // scalar's literal-1 and the EC point/group pins ride the bus). idx =
        // 0 falls out of the cursor reset, so it needs no separate constraint.
        builder.assert_zero(is_intro.clone() * (AB::Expr::ONE - is_boundary.clone()));
        builder.assert_zero(
            is_intro.clone() * local[COL_VAL_A].into() * (local[COL_VAL_A].into() - AB::Expr::ONE),
        );
    ...
    LB: LookupBuilder<F = Felt>,
{
    ...
    fn eval(&self, builder: &mut LB) {
        ...
        // Neg value cancel proof result slot.
        let neg_pai: LB::Expr = local[COL_NEG_PAI].into();
                    ...
                    |g| {
                        g.batch(
                            ...
                            |b| {
                                ...
                                // neg's value is grounded by the EC cancel
                                // proof `val_a + val = ∞`; the extra EcPoint
                                // consume pins the result slot to the group's
                                // point at infinity.
                                b.insert(
                                    "consume-ecgroupadd-neg",
                                    bnd_neg.clone(),
                                    EcGroupAddMsg {
                                        group_ptr: group_ptr.clone(),
                                        p_ptr: val_a.clone(),
                                ...
                                b.insert(
                                    "consume-ecpoint-neg-pai",
                                    bnd_neg.clone(),
                                    EcPointMsg {
                                        point_ptr: neg_pai.clone(),
                                        group_ptr: group_ptr.clone(),
                                        x_ptr: LB::Expr::ZERO,
                                        y_ptr: LB::Expr::ZERO,
                                        is_pai: LB::Expr::ONE,
                                ...
                                // Intro's base point, including the PAI flag,
                                // grounds `val = base` in the EC point store.
                                b.insert(
                                    "consume-ecpoint-intro",
                                    bnd_intro.clone(),
                                    EcPointMsg {
                                ...
                                // The `EcGroup` pin (sbound ↔ group): intro,
                                // combine, and neg.
                                b.insert(
                                    "consume-ecgroup",
                                    bnd_a.clone() + bnd_intro.clone(),
                                    EcGroupMsg {
                                        group_ptr: group_ptr.clone(),
                                        a_ptr: a_ptr.clone(),
                                        b_ptr: b_ptr.clone(),
