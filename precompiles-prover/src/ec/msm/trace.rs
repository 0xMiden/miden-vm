...
use super::{
    ...
    COL_IS_NEG, COL_J, COL_MULT, COL_NEG_PAI, COL_S_A, COL_S_B, COL_SBOUND_PTR, COL_SCALAR,
    COL_TAKE_A, COL_TAKE_B, COL_TAKE_BOTH, COL_VAL, COL_VAL_A, COL_VAL_B, EcMsmAir, NUM_MAIN_COLS,
};
use crate::{
...
struct ExprRecord {
    ...
    b_ptr: u32,
    bound_ptr: u32,
    // neg-only value-negation cell (0 on intro/combine): the point-at-infinity
    // result slot for the cancel proof.
    neg_pai: u32,
    rows: Vec<RowVals>,
...
impl EcMsmRequires {
    ...
    /// the store ptr of the value `1` under `sbound`. The point/group witness
    /// cells ground the expression in the EC stores. Returns the handle.
    ...
    /// modulus (to close the `EcGroup` consume pinning `sbound`). The value
    /// is grounded by the EC layer's cancel proof
    /// `EcGroupAdd(group, val_a, val, pai)`, with the ∞ result slot pinned
    /// by an `EcPoint(pai, is_pai = 1)` consume.
    /// Returns the handle.
    #[allow(clippy::too_many_arguments)]
    pub fn neg(
        &mut self,
    ...
    for (e_idx, e) in requires.exprs.iter().enumerate() {
        ...
        for (idx, rv) in e.rows.iter().enumerate() {
            ...
            if is_combine {
                ...
                set(COL_BOUND_PTR, e.bound_ptr);
                if is_boundary {
                    // The value negation lives on the boundary: the
                    // group-add cancel proof result slot.
                    set(COL_NEG_PAI, e.neg_pai);
                    let a_diff = expr_ptr - e.a_expr - 1;
                    set(COL_A_DIFF_LO, a_diff & 0xffff);
                    set(COL_A_DIFF_HI, a_diff >> 16);
                    bpl.require_range16((a_diff & 0xffff) as u16);
                ...
                // intro: the literal-1 scalar's two UintVal halves, plus the
                // EC point/group data used by the boundary lookup.
                set(COL_BASE_A, rv.base_a);
                set(COL_S_A, rv.s_a);
                set(COL_VAL_A, rv.base_b);
                set(COL_A_PTR, e.a_ptr);
                set(COL_B_PTR, e.b_ptr);
                set(COL_BOUND_PTR, e.bound_ptr);
...
mod tests {
    ...
    fn intro_constraints_hold() {
        ...
        let g = intro(&mut req, group, sbound, EcPointPtr::from_addr(3), one);
        let q = intro(&mut req, group, sbound, EcPointPtr::from_addr(4), one);
        req.consume_op(g, 2);
        req.consume_op(q, 1);
        local_check(req);
    }
    ...
    fn combine_constraints_hold() {
        ...
        let mut req = EcMsmRequires::new();
        let ga = intro(&mut req, group, sbound, g, one);
        let qb = intro(&mut req, group, sbound, q, one);
        let rows = vec![
            CombineRow {
                take_a: true,
                take_b: false,
    ...
    fn neg_constraints_hold() {
        ...
        let pai = EcPointPtr::from_addr(7);
        ...
        let n = req.neg(group, sbound, a_ptr, b_ptr, bound_ptr, c, gq, ngq, pai, neg_rows);
        req.consume_op(ga, 1);
        req.consume_op(qb, 1);
        req.consume_op(c, 1);
        req.consume_op(n, 1);
