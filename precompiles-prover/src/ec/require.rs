...
use crate::{
    ...
    math::{U256, add_reduce, mac_reduce, mod_inv, sub_reduce, to_limbs32},
...
impl<'a> EcRequire<'a> {
    ...
    pub fn create_group(&mut self, a: U256, b: U256, bound: UintPtr) -> (EcGroupPtr, EcPointPtr) {
        ...
        let group = self.create_group_at(a_ptr, b_ptr, bound);
        let pai = self.store.add_pai(group);
        (group, pai)
    }

    fn create_group_at(&mut self, a_ptr: UintPtr, b_ptr: UintPtr, bound: UintPtr) -> EcGroupPtr {
        if let Some(group) = self.store.group_by_curve(a_ptr, b_ptr, bound) {
            return group;
        }
        let b = self.uint.value(b_ptr);
        assert_ne!(b, U256::ZERO, "b = 0 puts (0,0) on the curve");
        let one = self.uint.intern(U256::ONE, bound);
        let b_inv = self.uint.intern(mod_inv(b, self.uint.value(bound)), bound);
        self.uint.mac_into(1, b_inv, b_ptr, 0, bound, one);
        let a_limbs = to_limbs32(self.uint.value(a_ptr));
        let bound_limbs = to_limbs32(self.uint.value(bound));
        self.store.create_group(a_ptr, b_ptr, bound, b_inv, one, a_limbs, bound_limbs)
    }
    ...
    pub fn constrain_scalar_bound(&mut self, group: EcGroupPtr, sbound: UintPtr) {
        self.store.set_scalar_bound(group, sbound, to_limbs32(self.uint.value(sbound)));
    }
    ...
    fn add_inner(
        ...
        // demands.
        let (case, r, transients, one_ptr, mints) = match (p_coords, q_coords) {
            (None, None) => {
                // ∞ + ∞: both pass flags ride the consumed tuples, and
                // the AIR's ties force `p = q = r`.
                assert_eq!(p, q, "∞ + ∞ takes the canonical PAI twice");
                (EcAddCase::PaiBoth, p, None, None, false)
            },
            (None, Some(_)) => (EcAddCase::PaiP, q, None, None, false),
            (Some(_), None) => (EcAddCase::PaiQ, p, None, None, false),
            (Some((px, py)), Some((qx, qy))) => {
                ...
                if x1 != x2 {
                    ...
                    // witness d⁻¹ certified by inv·d ≡ 1 — what pins λ
                    // to the unique chord slope.
                    let d = self.uint.sub(qx, px);
                    let one = self.uint.intern(U256::ONE, bound);
                    self.uint.require_uintval(one);
                    ...
                    let inv = self.uint.intern(d_inv, bound);
                    self.uint.mac_into(1, inv, d, 0, bound, one);
                    let (transients, r, fresh) = self.add_tail(d, lambda, inv, px, py, qx, group);
                    (EcAddCase::Generic, r, Some(transients), Some(one), fresh)
                    ...
                    let inv = self.uint.intern(mod_inv(y1, bound_v), bound);
                    self.uint.mac_into(1, inv, py, 0, bound, one);
                    self.uint.value_eq(px, qx);
                    self.uint.value_eq(py, qy);
                    let (transients, r, fresh) = self.add_tail(s, lambda, inv, px, py, qx, group);
                    (EcAddCase::Double, r, Some(transients), Some(one), fresh)
                }
            },
        };
    ...
    pub fn neg(
        ...
        if coords.is_none() {
            let pai = self.add_inner(group, p, p, mult);
            for _ in 0..mult {
                self.store.require_ecpoint(pai);
            }
            return (group, pai, pai);
        }
        let (px, py) = coords.expect("finite point");
        let (_, _, bound) = self.store.group_params(group);
    ...
    /// The live cases' shared tail: `w = λ²`, `t = x₁ + x₂`,
    /// `x₃ = w − t`, `e = x₁ − x₃`, `u = λ·e`, `y₃ = u − y₁`, with `R`
    /// minted as a **closure-cert** point — its membership rides this
    /// block's `EcOnCurveCert` (the group law is closed → on-curve
    /// operands give an on-curve result), so it pays *no* MAC trio.
    ...
    fn add_tail(
        ...
        let (_, _, bound) = self.store.group_params(group);
        let w = self.uint.mac(1, lambda, lambda, 0, bound);
        let t = self.uint.add(px, qx);
        let x3 = self.uint.sub(w, t);
        let e = self.uint.sub(px, x3);
        let u = self.uint.mac(1, lambda, e, 0, bound);
        let y3 = self.uint.sub(u, py);
        ...
        ([slope_aux, lambda, inv, t, w, e, u, x3, y3], r, mints)
    }
}
