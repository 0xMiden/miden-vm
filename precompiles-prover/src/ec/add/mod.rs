...
//! nonzero (`double`'s `y₁ ≠ 0`) are **inverse MACs against stored
//! field one**: a stored witness `inv` with
//!
//! ```text
//! inv·d + 0 ≡ 1      (generic: d = x₂ − x₁, the slope transient)
//! inv·y₁ + 0 ≡ 1     (double)
//! ```
//!
//! This makes either MAC unsatisfiable when the factor is zero — the
//! λ-float attack dies in the mul chiplet, deterministically, with no
//! β-dependent fingerprint, no aux witness registers, and no
//! completeness gap.
//!
...
/// Stored field-one ptr used by the generic/double inverse certificates.
pub const COL_ONE_PTR: usize = 11;
/// The case near-one-hot (cycle-constant; Σ = act + pai_p·pai_q).
pub const COL_PAI_P: usize = 12;
pub const COL_PAI_Q: usize = 13;
pub const COL_CANCEL: usize = 14;
pub const COL_DBL: usize = 15;
pub const COL_GEN: usize = 16;
pub const COL_ACT: usize = 17;
...
pub const COL_RP_LO: usize = 19;
pub const COL_RP_HI: usize = 20;
/// Limbs of `r_ptr − q_ptr − 1` — proving `r_ptr > q_ptr`.
pub const COL_RQ_LO: usize = 21;
pub const COL_RQ_HI: usize = 22;
    ...
    LB: LookupBuilder<F = Felt>,
{
    ...
    fn eval(&self, builder: &mut LB) {
        ...
        let one_ptr: LB::Expr = local[COL_ONE_PTR].into();
                    ...
                    |g| {
                        g.batch(
                            ...
                            |b| {
                                b.insert(
                                    "provide-ecgroupadd",
                                    act.clone() * neg_mult * at_res.clone(),
                                    EcGroupAddMsg {
                    ...
                    |g| {
                        g.batch(
                            ...
                            |b| {
                                ...
                                // inv·d ≡ 1 ⟹ d ≠ 0 — what pins λ to the
                                // unique chord slope.
                                b.insert(
                                    "consume-inv-d",
                                    generic.clone() * at_slope.clone(),
                    ...
                    |g| {
                        g.batch(
                            ...
                            |b| {
                                ...
                                // double's nonzero witness: inv·y₁ ≡ 1 —
        ...
        //             provide itself. The provide is consumed by `r`'s
        //             point-store row (`EcPointStore`), discharging its
        //             on-curve obligation without the MAC trio; the bus
        //             balances because a fresh result is minted by exactly
        //             one op.
        ...
        let one_bound: LB::Expr = local[COL_BOUND_PTR].into();
        let one_ptr_pin: LB::Expr = local[COL_ONE_PTR].into();
                        ...
                        g.batch(
                            ...
                            |b| {
                                b.insert(
                                    ...
                                    one_gate.clone(),
                                    UintValMsg {
                                        ptr: one_ptr_pin.clone(),
                                        bound_ptr: one_bound.clone(),
                                        offset: LB::Expr::ZERO,
                                        limbs: [
                                            LB::Expr::ONE,
                                            LB::Expr::ZERO,
