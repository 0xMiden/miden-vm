...
use super::{
    ...
    COL_DBL, COL_GEN, COL_MINTS, COL_ONE_PTR, COL_PAI_P, COL_PAI_Q, COL_PX, COL_PY, COL_QX, COL_QY,
    COL_RP_HI, COL_RP_LO, COL_RQ_HI, COL_RQ_LO, EcGroupAddAir, NUM_CELLS, NUM_MAIN_COLS, PERIOD,
...
pub(crate) struct EcAddOp {
    ...
    pub a: UintPtr,
    pub b: UintPtr,
    /// Stored field-one handle for the inverse certificates. Present only
    /// on generic/double blocks, where the gates read it.
    pub one: Option<UintPtr>,
    pub p: EcPointPtr,
    pub q: EcPointPtr,
    pub r: EcPointPtr,
    ...
    /// `(slope_aux, λ, inv, t)` on the slope row, `(w, e, u, x₃)` on the
    /// tail row, `y₃` opening the res row — `None` (all-zero cells) for
    /// the cases that allocate none.
...
fn op_block(op: &EcAddOp, mult: ProvideMult, ec: &EcStoreRequires) -> Vec<Felt> {
    ...
    set(ROW_RES, 0, transients[8]); // CELL_Y3
    ...
    for row in 0..PERIOD {
        ...
        set(row, COL_ONE_PTR, op.one.map_or(0, UintPtr::addr));
