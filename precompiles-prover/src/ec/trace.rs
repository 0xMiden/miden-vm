...
struct Group {
    ...
    a_limbs: [u32; 8],
    bound_limbs: [u32; 8],
    scalar_bound: Option<(UintPtr, [u32; 8])>,
}

/// A finite point's field-element handles: coordinates plus its membership
...
impl EcStoreRequires {
    ...
    /// the group's handle.
    pub fn group_by_curve(&self, a: UintPtr, b: UintPtr, bound: UintPtr) -> Option<EcGroupPtr> {
        self.by_curve.get(&(a, b, bound)).copied()
    }
    ...
    pub fn set_scalar_bound(&mut self, group: EcGroupPtr, sbound: UintPtr, limbs: [u32; 8]) {
        let entry = &mut self.groups[group.0 as usize - 1].scalar_bound;
        match entry {
            None => *entry = Some((sbound, limbs)),
            Some((prev, _)) => assert_eq!(*prev, sbound, "conflicting scalar bound for the group"),
        }
    }

    /// Route the UintVal consumers emitted by live group rows. This runs after all group
    /// scalar-bound constraints have been recorded, but before the UintStore trace reads its
    /// demand ledger.
    pub fn route_uintval_demands(&self, uint: &mut UintStoreRequires) {
        for group in &self.groups {
            uint.require_uintval(group.a);
            uint.require_uintval(group.one);
            uint.require_uintval(group.scalar_bound.map_or(group.bound, |(ptr, _)| ptr));
        }
    }
...
fn groups_trace(requires: &EcStoreRequires) -> RowMajorMatrix<Felt> {
    ...
    for i in 0..height {
        ...
        if let Some(group) = requires.groups.get(i) {
            ...
            let (sbound, sbound_limbs) =
                group.scalar_bound.unwrap_or((group.bound, group.bound_limbs));
            row[G_COL_SBOUND_PTR] = Felt::from(sbound.addr());
            row[crate::ec::groups::COL_B_INV_PTR] = Felt::from(group.b_inv.addr());
            row[crate::ec::groups::COL_ONE_PTR] = Felt::from(group.one.addr());
            for (i, limb) in group.a_limbs.iter().enumerate() {
                row[crate::ec::groups::COL_A_LIMBS_LO + i] = Felt::from(*limb);
            }
            for (i, limb) in sbound_limbs.iter().enumerate() {
                row[crate::ec::groups::COL_SBOUND_LIMBS_LO + i] = Felt::from(*limb);
            }
            row[G_COL_MULT] =
                Felt::from(requires.group_demand.get(&EcGroupPtr(ptr)).copied().unwrap_or(0));
            row[crate::ec::groups::COL_ACT] = Felt::ONE;
        }
        vals.extend(row);
    }
