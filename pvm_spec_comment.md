...
*Layout amendment: sponge state reordered to Miden-native `[RATE, CAPACITY]` convention (see §3 below). No cryptographic change; pure relabeling.*

---

## What this is

An AIR specification for the **Precompile VM (PVM)**: a second STARK proof system that runs alongside Miden and verifies *cryptographic transcripts* — trees of typed commitments to field elements, group elements, Keccak digests, and assertions.

Miden programs build transcripts during execution (as content-addressed DAGs of commitment hashes), and the root of the transcript becomes a public input of the Miden execution proof. That same root is then the public input of a **second proof**, generated from this AIR, attesting: *"every assertion committed to this root evaluates to `True` under a fixed set of algebraic and hashing rules."* Together the two proofs establish that a Miden program was executed correctly **and** that every cryptographic claim it made is semantically valid.

A companion comment (posted as a reply) describes the Miden-side architecture for *authoring* transcripts — expression DAGs, event handlers, canonical pointer allocation, witness generation. **This comment is the verification side.**

## Architectural shape

One **eval chip** takes a transcript root hash and recursively resolves it into a typed binding. Per node:

1. Look up the node's 12-felt preimage via a Hash bus lookup against the permutation chip.
2. Decode the tag + param fields to determine the node type.
3. Recursively resolve children if present; dispatch to an auxiliary chip for arithmetic / group ops / Keccak hashing.
4. Provide a typed binding `(hash → value)` back onto the Binding bus.

Five auxiliary chips, each deliberately narrowly scoped:

- **Permutation chip** — bulk Poseidon2 permutations. No semantic awareness of tags or versions.
- **Field chip** — canonical field-element table + Add / Sub / Mul.
- **Group chip** — canonical group-element table + curve Add / Sub.
- **Chunk chip** — sponge construction for variable-length byte data.
- **Keccak chip** — Keccak-256 over chunk sequences.

All inter-chip communication is **LogUp buses** over fixed-width tuples — nine buses total (widths 2 through 16); see the §9 summary below for the full list of providers and consumers.

## Design choices worth calling out upfront

- **Content-addressed DAG.** Node identity is the 4-felt RPO hash of its preimage. Structurally identical subexpressions produce the same hash and collapse automatically — no duplication in the witness, no explicit sharing markers needed.
- **Version-gated spec upgrades.** Every preimage carries a `CURRENT_VERSION` field in the capacity. Bumping the version invalidates every prior proof, enabling cleanly-breaking spec changes.
- **Canonical pointer assignment for `Eq`.** Field and group chips guarantee equal values share the same opaque `ptr`. `FieldBinOp::Eq` and `GroupBinOp::Eq` then reduce to a single `lhs_ptr == rhs_ptr` check — cheap and sound.
- **Symmetric `lhs` / `rhs` in Transcript nodes.** A transcript is a left-leaning chain of assertion hashes; the evaluator walks backward through the chain until it hits `ZERO_HASH = [0, 0, 0, 0]`. Canonical assertion ordering is a compiler concern, not a verifier concern.
- **No division, no scalar multiplication.** The transcript compiler translates `a / b = c` into a witnessed `b * c = a`, and scalar multiplication into repeated group additions. Keeps the AIR small.
- **Miden-native sponge state layout** (per the amendment note above). The preimage has rate at `state[0..8]` and capacity at `state[8..12]`, matching Miden's hasher chiplet verbatim. Hash output is the first rate word post-permutation (`state[0..4]` = `DIGEST_RANGE` = `RATE0'`).

## What the PVM can express

A transcript is a tree whose **leaves** are:

- **Field elements** — in four `field_ty` flavors (base / scalar × curve A / B). Stored u16-limb internally, exposed u32-LE in the preimage.
- **Group elements** — on one of two curves, in affine coordinates. Point at infinity encoded as `(0, 0)`.
- **Keccak digests** — 256-bit, u32-LE.

And whose **internal nodes** are operations:

- **FieldBinOp** — Add / Sub / Mul / Eq over two field elements of the same type.
- **GroupCreate** — builds a group element from an (x, y) coordinate pair, with on-curve enforcement.
- **GroupBinOp** — Add / Sub / Eq over two group elements of the same type.
- **Keccak** — commits a digest + a chunk sequence + a `len_bytes` count to a single node.
- **Transcript** — a left-leaning chain of assertion hashes terminating in `ZERO_HASH`.

The full tag enumeration, parameter constraints, operation semantics, and per-tag dispatch logic are in the collapsed sections below.

---

## Details

The rest of the spec is collapsed for readability. Expand any section relevant to your review focus.

<details>
<summary><strong>§2 Type system — field types, encoding, group types, value variants</strong></summary>
...
<details>
<summary><strong>§3 Hash preimage layout — the 12-felt Miden-native state</strong></summary>
...
<details>
<summary><strong>§4 Tag enumeration — eight node types and parameter constraints</strong></summary>
...
<details>
<summary><strong>§5 Operations — field and group arithmetic semantics</strong></summary>
...
<details>
<summary><strong>§6 Eval chip logic — per-tag dispatch arms</strong></summary>
...
<details>
<summary><strong>§7 Bus relations — nine LogUp tuples linking chips</strong></summary>
...
<details>
<summary><strong>§8 Auxiliary chips — permutation, field, group, chunk, Keccak</strong></summary>
...
<details>
<summary><strong>§9 Bus summary (quick reference)</strong></summary>
